...
- **Read colours by role from `lib/theme/arul_tokens.dart` (`ArulTokens`).** `lib/app/theme/tokens.dart`
  (`ArulColors`) is the LEGACY ladder; its values are remapped onto the same palette, but
  **new code must not grow it**.
- **No literal `Color(0x…)` in screens.** Two exceptions already exist in the tree and nothing else:
  ...
  sentinel.
- **Schemes are hand-specified, NOT `ColorScheme.fromSeed`** (it invents secondary/tertiary).
