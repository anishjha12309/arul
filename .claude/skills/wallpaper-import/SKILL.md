...
2. **Title offset:** query Neon for `max(N)` in titles matching `^<Cat> ([0-9]+)$`; number the new ones from `N+1` so labels never collide.
...
9. **Import:** `node ROOT/import.mjs` — R2 PUT (media + thumbs) → one Neon txn (rows + `content_version` bump) → build-catalog.
