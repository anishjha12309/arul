...
- [ ] Ordering model (3 tiers: `feed_rank` ASC nulls-last → use-count DESC → catalog position; one pure `feedOrder()`/`orderedByUse()` on EVERY chip) is owned by CLAUDE.md §5b. Pinned here: category order = `sort_order ASC, created_at DESC, id ASC`; the last tie MUST break on catalog position because `List.sort` is NOT stable and `_syncFeed` compares served lists by ordered ids, so an unstable order re-points the pager under a scrolling user; zero pins + zero counts ⇒ every chip is byte-identical to catalog order, which build-catalog emits CATEGORY-INTERLEAVED so one import cannot own the first screen (`interleaveByCategory` — idempotent, never reorders within a category, so chips are unaffected)
- [ ] A pin is NULL for ~every row — unpinned, never rank 0, and it must sort BELOW every pin. Folding NULL to 0 pins the whole uncurated catalog above the curated head. An import writes no rank, so pins survive a bulk drop; that is the property that makes curation safe, and the reason it must never move back into `sort_order`. A catalog cached before the field existed parses as nothing-pinned and degrades to the popularity order rather than failing to parse
