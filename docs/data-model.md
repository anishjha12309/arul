...
**wallpapers:** id(PK) · title · type(static|live — a **rendering hint, never a filter**) · **category** (first-class Arul delta: `amman|ayyappan|murugan|perumal|sivan|temples`, free text + index so new categories need no migration — **ringtones use a different six: the five deities, minus `temples`, plus `others`**, and each tab derives its chips from its own catalog, so the two differing is correct) · tags[] (free-form extras) · full_key(R2, public) · mime · duration_ms(null for static) · width · height · bytes · is_published · sort_order (imports leave it at the default 0) · created_at · **apply_count** (bigint, default 0 — orders the All chip) · ~~feed_rank~~ (RETIRED 2026-08-13: column left in place, nothing reads it, build-catalog drops it from the JSON). No is_premium — the gate is in the Worker.
