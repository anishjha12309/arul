...
`ms_since_authenticate` and `surface`. `login_cancelled` adds `nudge` (the outcome, so the LINE the
user saw) and `ms_to_surface`, carried by `login_success` too — the slow-surface split needs a
