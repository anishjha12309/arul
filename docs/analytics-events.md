...
- **PostHog** — **every install**, allow-listed journey events + SDK lifecycle. Two gates remain: `AnalyticsCohort` (is this install in the panel? — currently all of them) and `AllowlistedAnalyticsService` (is this event on the list?).
...
## PostHog cost model — why the two gates still exist

PostHog bills per event on a 1M/month free tier. Sampling is a **cost control, and there is no cost
to control until the install base produces one** — the 5% panel this replaced (2026-08-13) was sized
for 800k MAU and, at the app's real scale, meant a panel of roughly ONE device that went days without
