...
**Order differs per chip, from ONE function — `feedOrder()`/`orderedByUse()` in `catalog_providers.dart` (both tabs).** A category = catalog order (newest first). **All = `apply_count`/`set_count` DESC, ties in catalog order** — so at zero data it collapses to plain newest-first, which IS the intended default. Ties MUST break on catalog position (`List.sort` is not stable, and an unstable order re-points the pager + video pool under a scrolling user); keep it a pure function of catalog content, and resolve apply-restore indices through it. Counts come from Neon via `/media/signed-url`, NEVER from analytics — sampled tools are not a ranking source. They reach users only on a rebuild, so the daily cron bumps `content_version` when a count moved. Accepted cost of newest-first-at-zero: a bulk import (one transaction, tied `created_at`) owns the first screen until applies accrue — this replaced an FNV-1a shuffle written to prevent exactly that. `feed_rank` and the CMS Feed-order page are RETIRED (column left in Neon, dropped in build-catalog). Contracts: [docs/edge-cases.md](docs/edge-cases.md) §Browse.
