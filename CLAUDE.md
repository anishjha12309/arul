...
**Order differs per chip, from ONE function — `feedOrder()` in `catalog_providers.dart`.** A category = catalog order (newest first). **All = a stable shuffle** hashed on the row id, because a bulk import is one transaction whose rows tie on `sort_order`+`created_at` and otherwise seize the whole first screen. Keep it a pure function of catalog content (never `List.shuffle`/time/`content_version`) or the feed re-points under a scrolling user, and resolve apply-restore indices through it. Contracts: [docs/edge-cases.md](docs/edge-cases.md) §Browse.
