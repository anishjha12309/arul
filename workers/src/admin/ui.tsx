/**
 * Arul CMS — shared server-rendered UI (Hono JSX + HTMX).
 *
 * No client build step: pages are rendered on the Worker and progressively
 * enhanced with HTMX + a little vanilla JS. Brand: dark glass chrome with a
 * temple-gold brand accent, Strapi-style admin layout — grouped sidebar,
 * card/table content, accessible native-<dialog> modals.
 *
 * IMPORTANT — this file is presentation only. It does NOT change any form
 * `action`, `method`, field `name`, or `data-*` attribute the upload script and
 * the POST handlers depend on. The upload <script> (UPLOAD_JS) is kept verbatim
 * and lives once per page (in Layout) so it works for forms inside modals too
 * (it delegates off `document` submit). The only behavioural change is that
 * destructive buttons now use a `data-confirm` modal instead of window.confirm.
 */
...
const CSS = `
/* ════════════════════════════════════════════════════════════════════════
   Arul CMS — Apple "Liquid Glass" design system (DARK ONLY).
   Frosted translucent surfaces (backdrop-filter), hairline borders, soft
   layered shadows, SF system type, Apple system-colour accents. Class
   vocabulary is unchanged so every page keeps working — this is a restyle.
   ════════════════════════════════════════════════════════════════════════ */
:root{
  color-scheme:dark;
  ...
  /* ── Brand tie-in ── */
  --brand:#d4a017;           /* Arul temple gold — login accent bar */
...
/** Sidebar navigation, grouped Strapi-style. */
const NAV_GROUPS: { label: string; items: { key: string; href: string; label: string; ic: string }[] }[] = [
  {
    label: "Overview",
    items: [{ key: "dashboard", href: "/admin", label: "Dashboard", ic: "▣" }],
  },
  {
    label: "Content",
    items: [
      { key: "wallpapers", href: "/admin/wallpapers", label: "Wallpapers", ic: "▦" },
      { key: "submissions", href: "/admin/submissions", label: "Submissions", ic: "⇪" },
    ],
  },
  {
    label: "Settings",
    items: [{ key: "config", href: "/admin/config", label: "App config", ic: "⚙" }],
  },
];

/**
 * The shared destructive-confirmation dialog + the modal/list controller scripts.
 * Rendered once per page inside Layout so every page has exactly one of each.
 *
 * MODAL_JS:  opens dialogs via [data-dialog-target] (resetting a linked hx-target
 *            to a Loading… placeholder so a re-opened modal never flashes the
 *            previous row), closes via [data-dialog-close] / backdrop click, and
 *            replaces window.confirm with the #confirm-dlg modal for any
 *            <form data-confirm="…">. Runs the confirm interceptor in the CAPTURE
 *            phase + stopImmediatePropagation so it pre-empts the bubble-phase
 *            upload-script submit handler (delete forms are never upload forms,
 *            so there is no real conflict — this is belt-and-suspenders).
 * LIST_JS:   client-side search / filter / sort / pagination over already-rendered
 *            rows. Only toggles row visibility + reorders DOM — never touches form
 *            submission. No backend awareness.
 */
const MODAL_JS = `
(function(){
...
/**
 * NAV_JS: mobile off-canvas drawer toggle. Adds/removes `nav-open` on <body>
 * (CSS slides the glass sidebar in/out). Opens via [data-nav-toggle], closes via
 * the scrim ([data-nav-close]), any sidebar link click, or Esc. Presentation
 * only — touches no form, action, or data hook.
 */
const NAV_JS = `
(function(){
...
/** Top-level page chrome with the grouped sidebar nav + shared modal/scripts. */
  ...
  return (
    <html lang="en">
      <head>
        ...
        <title>{props.title} · Arul CMS</title>
...
/**
 * Accessible modal built on the native <dialog> element (opened with
 * showModal() by MODAL_JS → real focus trap, Esc-to-close, ::backdrop, page
 * inert, focus return — all for free). The body holds the (unchanged) forms;
 * because the upload script delegates off document, forms inside here work as-is.
 */
 ...
 * <input type=file data-slot="X">, it presigns via /admin/media/upload-url,
 * PUTs the bytes directly to R2 (zero-egress), then fills hidden fields
 * key_X / mime_X / id_X and submits the form. File inputs carry no `name`, so
 * the bytes are never posted to the Worker (upload completes before the DB row).
 *
 * Delegated off `document` submit, so it also handles forms rendered inside a
 * <dialog> / swapped in by HTMX. KEEP THIS SCRIPT BYTE-FOR-BYTE.
 * (Arul delta: the presign body also carries the form's `category` value —
 * canonical keys are category-partitioned, see admin/media.ts prefixFor.)
 */
const UPLOAD_JS = `
(function(){
  ...
  async function run(form){
    ...
    for(var i=0;i<inputs.length;i++){
      ...
      var pres=await fetch('/admin/media/upload-url',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({kind:form.dataset.kind,slot:input.dataset.slot,contentType:ct,size:f.size,category:cat})});
...
export const UploadScript: FC = () => <script dangerouslySetInnerHTML={{ __html: UPLOAD_JS }} />;
