...
void main() {
  ...
  group('past the launch', () {
    testWidgets(
      'a wallpaper tap over a premium screen opened by a push lands on the feed',
      (tester) async {
        ...
        expect(
          ...
          reason:
              'parked for the feed to consume once its catalog resolves the id',
        );
      },
    );
    ...
    testWidgets('a category is selected BEFORE the feed is routed to', (
      tester,
    ) async {
      ...
      expect(
        ArulDeepLink.pendingTarget,
        isNull,
        reason: 'a category is selected, never parked',
      );
    });

    testWidgets('a premium tap opens the paywall stamped source=push', (
      tester,
    ) async {
      await pumpApp(tester, initial: '/ringtones');
  ...
  group('during the launch', () {
    testWidgets('a cold tap on the splash is held until the splash routes', (
      tester,
    ) async {
      ...
      expect(
        find.text('splash'),
        findsOneWidget,
        reason: 'the auth decision is not in yet',
      );
    ...
    testWidgets('a signed-out cold tap never passes the sign-in wall', (
      tester,
    ) async {
      await pumpApp(tester, initial: '/');
      ...
      expect(
        location(),
        '/premium?source=push',
        reason: 'last tap wins, applied once',
      );
      ...
      expect(
        location(),
        '/ringtones',
        reason: 'the held tap does not replay on later navigation',
      );
    });

    testWidgets(
      'a held wallpaper tap is parked at once for the shell to follow',
      (tester) async {
  ...
  test('locationFor sends content to its tab and premium to the paywall', () {
    expect(
      PushTapRouter.locationFor(const WallpaperLinkTarget('w')),
      '/browse',
    );
    expect(
      PushTapRouter.locationFor(const CategoryLinkTarget('sivan')),
      '/browse',
    );
    expect(
      PushTapRouter.locationFor(const RingtoneLinkTarget('r')),
      '/ringtones',
    );
    expect(
      PushTapRouter.locationFor(const TabLinkTarget(ArulTab.ringtones)),
      '/ringtones',
    );
    expect(
