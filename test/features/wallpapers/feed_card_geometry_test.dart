...
/// Pins the reel card's shape — `FeedCardGeometry` is a pure function of the
/// screen and the reel, so this is the cheapest place to catch a knob moving.
///
/// The shape is a product decision (Shubh's tile numbers on Arul's reel — see
/// the class doc). Anything that moves these numbers is either a deliberate
/// re-decision or a regression; there is no third case.
void main() {
  /// The reel height the feed's LayoutBuilder hands in, modelled from a
  /// MEASURED frame rather than summed from tokens: on a Nothing A001
  /// (411.4×911.2 dp, 48 dp status bar, 24 dp gesture inset — 2026-08-25
  /// screenshot) the card's top edge sat at 435 px, i.e. 118 dp of chrome
  /// below the status bar (title row + chips), and the reel's bottom at 2129
  /// px, i.e. 76 dp above the gesture inset. The previous model (107 dp of
  /// chrome, nothing below) predicted a 682 dp card where the phone showed 601.
  ...
  group('on the Nothing A001 — the Shubh reference phone', () {
    // 1080×2392 at dpr 2.625 → 411.43 × 911.24 dp.
    const dpr = 2.625;
    ...
    test('the card is the size of Shubh\'s tile on the same phone', () {
      // Shubh, measured from its accessibility tree on this phone: the pager
      // runs 441..2160 px (1719), a page is 0.95 of that (1633), and the tile
      // is the page minus 8 dp (21 px) top and bottom → 996 × 1591 px.
      ...
      // By hand: reel = 911.24 − 48 − 118 − 76 − 24 = 645.24; the card gets
      // 645.24 − 16 − 25 = 604.24 dp = 1586 px. Within 2 dp of Shubh's 1591.
    ...
    test('the card is height-clamped, not aspect-driven, on this phone', () {
      // `width * cardAspect` does not fit, so the solver hands back whatever is
      // left after the gap and the minimum peek — which is what makes `gutter`
      // a width-only knob.
      expect(
  ...
  group('on a 1080x2400 phone', () {
    ...
    test('the card is 361 x 598, at ~1:1.66', () {
      // By hand: width = 392.7 − 2·16 = 360.7. The reel is 872.7 − 24 − 118 −
      // 76 − 16 = 638.7; the requested 360.7 × 1.86 = 670.9 does not fit, so
      // height = 638.7 − 16 − 25 = 597.7.
  ...
  group('the crop is understood, not accidental', () {
    ...
    test('the REALISED card is squarer than 9:16, so the trim is top/bottom', () {
      // Shubh's tile is shorter than the artwork on every ordinary phone, so
      // `cover` matches WIDTH and overflows height: ~7% on a 1080×2400, ~10% on
      // the A001. That is the expensive direction for devotional art, which is
      // exactly why ViewerMedia.cropAlignment's upward bias must stay — it is
      // LIVE now, not dormant, and keeps the trim off the crown.
    ...
    test('the card still ASKS for a taller-than-source shape', () {
      // The reel squeezes it, but the request stays on the safe side of the
      // boundary — on a screen tall enough to grant it the crop is horizontal.
  ...
  group('short screens degrade in the right order', () {
    ...
    test('a small screen ends up far SQUARER than 9:16 — the upward crop '
      ...
      // By hand: reel 640 − 24 − 118 − 76 − 16 = 406; height 406 − 16 − 25 =
      // 365 over width 360 − 32 = 328 → 1.11.
      final geo = FeedCardGeometry.solve(screen: small, reelHeight: smallReel);
