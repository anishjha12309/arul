// GENERATED by tools/l10n/gen_arb_index.dart — do not edit.
//
// Regenerate after ANY ARB change:
//     dart run tools/l10n/gen_arb_index.dart

/// Every localized string, by locale then by ARB key.
///
/// A locale map is SHORTER than English when keys have been demoted to
/// English-everywhere: a demoted key is absent from the five non-English
/// ARBs, and gen_l10n back-fills the template value at runtime.
library;

const Map<String, Map<String, String>> kArbStrings = {
  'en': {
    'appName': 'Arul',
    'appTagline': 'SOUTH INDIAN WALLPAPERS',
    'categoryAll': 'All',
    'feedLiveBadge': 'Live',
    'feedEmptyTitle': 'Nothing here yet',
    'feedEmptyBody': 'Try another category.',
    'feedErrorTitle': 'Couldn\'t load wallpapers',
    'feedErrorBody': 'Check your connection and try again.',
    'retry': 'Retry',
    'errorGeneric': 'Something went wrong.',
    'signInCaption': 'Bring the divine home',
    'signInGoogle': 'Continue with Google',
    'signInSubtitleIdle': 'Choose an account to start',
    'signInSubtitleExchanging': 'Signing you in…',
    'signInNudgeBackedOutQuick': 'Didn\'t go through? Tap again',
    'signInNudgeBackedOutSlow': 'Tap once and wait for Google',
    'signInNudgeNeverOpened': 'Google didn\'t open. Tap again',
    'signInNudgeAddAccount': 'Use an account on this phone',
    'signInNudgeReauth': 'Tap again and pick an account',
    'signInNudgeActivityClosed': 'Sign-in closed early. Tap again',
    'signInNudgeNoProvider': 'Update Google Play services',
    'signInFixBackedOutSlow': 'Google is taking a while on this phone.',
    'signInFixAddAccount':
        'No new account needed. Any Google account already on this phone works.',
    'signInFixReauth':
        'Google couldn\'t verify that account. Re-sign in to Google in phone settings.',
    'signInFixActivityClosed':
        'Google Play services closed the sign-in window.',
    'signInLinkAccountSettings': 'Open account settings',
    'signInLinkPlayServices': 'Check Google Play services',
    'signInLinkPlayStore': 'Update in Play Store',
    'signInTermsLink': 'Terms',
    'signInPrivacyLink': 'Privacy',
    'premiumTitle': 'Premium',
    'premiumHeadline': 'Unlock every wallpaper',
    'premiumSub': 'Browsing is always free. Premium is for making them yours.',
    'premiumBenefitApply': 'Apply any wallpaper, static or live',
    'premiumBenefitLive': 'Live video wallpapers in full quality',
    'premiumBenefitShare': 'Share wallpapers with friends and family',
    'premiumBenefitNew': 'New wallpapers added every week',
    'premiumPrice': '₹199 / month',
    'premiumCta': 'Start free trial',
    'premiumTrialNote':
        'One free trial per account. Cancel anytime — you keep access until the period ends.',
    'premiumComingSoon': 'Payments arrive with the backend.',
    'settingsTitle': 'Settings',
    'settingsAppearance': 'Appearance',
    'themeSystem': 'System',
    'themeLight': 'Light',
    'themeDark': 'Dark',
    'settingsContent': 'Content',
    'settingsAbout': 'About',
    'settingsSupport': 'Need help',
    'settingsPrivacy': 'Privacy policy',
    'settingsVersion': 'Version',
    'uploadTitle': 'Upload your content',
    'uploadBody':
        'Share your own wallpaper with the community. We review every submission before it goes live.',
    'uploadPickCategory': 'Choose a category',
    'uploadPickFile': 'Choose a file',
    'uploadSpecNote':
        'Photos: 1080×1920. Videos: 1024×1824, no audio, under 50 MB.',
    'uploadComingSoon': 'Uploads arrive with the backend.',
    'apply': 'Apply',
    'share': 'Share',
    'applyTargetTitle': 'Set wallpaper on',
    'applyTargetHome': 'Home screen',
    'applyTargetLock': 'Lock screen',
    'applyTargetBoth': 'Home and lock screen',
    'applied': 'Wallpaper applied',
    'appliedLiveFallback':
        'Live wallpapers aren\'t supported on this phone, so we set a still image instead.',
    'offlineBody': 'You\'re offline. Check your connection and try again.',
    'offlineTitle': 'No internet',
    'offlineFeedBody': 'Turn on the internet to see wallpapers.',
    'wallpaperShareCaption':
        'More devotional wallpapers like this one — still and live — on Arul:\n{link}',
    'referShareMessage':
        'I\'ve been using Arul for South Indian devotional wallpapers — Amman, Murugan, Perumal, Sivan, and live ones that actually move. Thought you\'d like it.\n\n{link}',
    'tabWallpapers': 'Wallpapers',
    'tabRingtones': 'Ringtones',
    'earn': 'Earn',
    'ringtoneSet': 'Set',
    'ringtonePreviewSemantic': 'Preview',
    'ringtonePreviewUnavailable': 'Preview not available yet',
    'ringtonesEmptyTitle': 'Ringtones are coming soon',
    'ringtonesEmptyBody':
        'Devotional ringtones are on their way. Check back soon.',
    'ringtonesErrorTitle': 'Couldn\'t load ringtones',
    'ringtoneSetSuccess':
        'Ringtone set. If it doesn\'t appear, restart your phone.',
    'ringtoneSetFailed': 'Couldn\'t set the ringtone. Please try again.',
    'ringtoneSetPreparing': 'Preparing…',
    'ringtoneSetDownloading': 'Downloading…',
    'ringtoneSetApplying': 'Setting ringtone…',
    'save': 'Save',
    'cancel': 'Cancel',
    'errorGenericRetry': 'Something went wrong. Please try again.',
    'settingsFallbackName': 'Your account',
    'settingsFallbackEmail': 'Signed in with Google',
    'settingsPremiumSubLocked': 'Unlock apply & share',
    'settingsPremiumSubTrial': 'You\'re on the free trial',
    'settingsPremiumSubCancelled': 'Auto-renew off · access continues',
    'settingsPremiumSubActive': 'You\'re a member',
    'settingsReferSub': 'Earn 30 days free premium',
    'settingsTellFriend': 'Tell a friend',
    'settingsTellFriendSub': 'Send Arul to someone who would love it',
    'settingsRemindersSubOn': 'Weekly and festival reminders on',
    'settingsRemindersSubOff': 'Festival and weekly reminders',
    'settingsLanguage': 'Language',
    'settingsTheme': 'Theme',
    'settingsNeedHelp': 'Need help?',
    'settingsNeedHelpSub': 'Contact support',
    'settingsUpload': 'Upload your content',
    'settingsUploadSub': 'Share a wallpaper or ringtone',
    'settingsLogout': 'Logout',
    'settingsLogoutConfirmTitle': 'Logout?',
    'settingsLogoutConfirmBody': 'You can sign back in anytime with Google.',
    'settingsDeleteAccount': 'Delete account',
    'settingsDeleteConfirmTitle': 'Delete account?',
    'settingsDeleteConfirmBody':
        'This removes your account, favourites and rewards for good.',
    'settingsDeleteConfirmBodyPremium':
        'This removes your account, favourites and rewards for good.\n\nYour Arul Premium subscription will be cancelled and any time left on it is lost — no refund. Signing up again will not restore it, and you will not get another free trial.',
    'settingsTerms': 'Terms & Conditions',
    'settingsRefund': 'Refund policy',
    'settingsSupportEmailPrompt': 'Please describe your issue or feedback:',
    'settingsSupportEmailDetails':
        'The details below help us resolve your request faster:',
    'settingsNoEmailApp': 'No email app found. Write to us at {email}',
    'settingsEditNameTitle': 'Your name',
    'settingsEditNameSub': 'Shown on wallpapers you upload',
    'themeSystemDefault': 'System default',
    'themeSystemSub': 'Follow device setting',
    'themeLightSub': 'Ivory & silk',
    'themeDarkSub': 'Lamp-lit maroon',
    'remindersTitle': 'Reminders',
    'remindersToggleTitle': 'Devotional reminders',
    'remindersToggleSub': 'The weekly day, and every major festival',
    'remindersScheduleNote':
        'You\'ll get one reminder each week on Velli Kizhamai, and one a few days before each major festival — Pongal, Deepavali, Navaratri, Sivarathiri and the rest. Around two a month.',
    'remindersPermissionToast':
        'Notifications are off for Arul. Turn them on in your phone settings to get reminders.',
    'remindersTimeLabel': 'Reminder time',
    'remindersComingUp': 'Coming up',
    'remindersMonthJan': 'Jan',
    'remindersMonthFeb': 'Feb',
    'remindersMonthMar': 'Mar',
    'remindersMonthApr': 'Apr',
    'remindersMonthMay': 'May',
    'remindersMonthJun': 'Jun',
    'remindersMonthJul': 'Jul',
    'remindersMonthAug': 'Aug',
    'remindersMonthSep': 'Sep',
    'remindersMonthOct': 'Oct',
    'remindersMonthNov': 'Nov',
    'remindersMonthDec': 'Dec',
    'premiumBrandTitle': 'Arul Premium',
    'premiumScreenSubline': 'The full collection, alive on your screen',
    'premiumPerkEvery': 'Every wallpaper, still and live',
    'premiumPerkApplyShare': 'Apply and share without limits',
    'premiumPerkNew': 'New arrivals every week',
    'premiumPerMonth': '/ month',
    'premiumPlanNote': 'UPI Autopay · cancel anytime',
    'premiumTrialPill': '1 DAY FREE',
    'premiumCtaPaid': 'Get Premium',
    'premiumFootnoteTrial':
        'Free for 1 day, then {price}/month. UPI Autopay verifies your account with ₹2, refunded instantly. Browsing stays free forever.',
    'premiumFootnotePaid':
        '{price} charged today, then renews monthly via UPI Autopay. Cancel anytime. Browsing stays free forever.',
    'premiumComingSoonToast': 'Premium is coming soon.',
    'premiumWelcomeToast': 'Welcome to Arul Premium!',
    'premiumCelebrateTitle': 'You\'re in',
    'premiumCelebrateBody':
        'Arul Premium is active. Know someone who would love these wallpapers? Send them one.',
    'premiumSheetPitch':
        'Every wallpaper, live and still. Apply and share freely across all six categories.',
    'premiumKeepBrowsing': 'Keep browsing free',
    'referTitle': 'Refer & Earn',
    'referHeroTitle': 'Gift a friend, earn a month',
    'referHeroBody':
        '30 days of free premium for every friend who subscribes with your link',
    'referShareWhatsapp': 'Share via WhatsApp',
    'referRewardsLabel': 'Rewards earned',
    'referRewardDays': '{days} days',
    'referHowItWorks': 'How it works',
    'referStep1': 'Share your link with friends and family',
    'referStep2': 'They install Arul and subscribe to premium',
    'referStep3': '30 days of free premium lands in your account',
    'referEmpty': 'No referrals yet — your first friend is one share away',
    'referShareCta': 'Share Arul',
    'referNotNow': 'Not now',
    'uploadScreenTitle': 'Upload wallpaper',
    'uploadPickZoneTitle': 'Choose an image or video',
    'uploadPickZoneSub': 'Portrait, 1080×2400 or larger',
    'uploadTitleLabel': 'Title',
    'uploadTitleOptional': '(optional)',
    'uploadTitleHint': 'e.g. Meenakshi at dusk',
    'uploadCategoryLabel': 'Category',
    'uploadRightsCheckbox':
        'I own the rights to this content or have permission to share it',
    'uploadSubmitCta': 'Submit for review',
    'uploadFootnote': 'Approved wallpapers appear in the feed with your name',
    'uploadRejectStatic': 'Please choose a JPEG, PNG or WebP image.',
    'uploadRejectLive': 'Please choose an MP4 video.',
    'uploadRejectAudio': 'Please choose an MP3, AAC or M4A audio file.',
    'uploadKindLabel': 'What are you sharing?',
    'uploadKindWallpaper': 'Wallpaper',
    'uploadKindRingtone': 'Ringtone',
    'uploadPickZoneTitleAudio': 'Choose an audio file',
    'uploadPickZoneSubAudio': 'MP3, AAC or M4A',
    'uploadTitleHintRingtone': 'e.g. Kanda Sasti Kavasam',
    'uploadFootnoteRingtone':
        'Approved ringtones appear in the Ringtones tab with your name',
    'uploadShareMomentBodyRingtone':
        'We\'ll review your ringtone shortly. While you wait — know someone who would enjoy Arul?',
    'uploadTooLarge': 'File is too large (max {max}).',
    'uploadSuccessToast': 'Submitted for review — thank you!',
    'uploadShareMomentTitle': 'Thank you',
    'uploadShareMomentBody':
        'We\'ll review your wallpaper shortly. While you wait — know someone who would enjoy Arul?',
    'uploadComingSoonToast': 'Upload is coming soon.',
    'premiumNavTitle': 'SUBSCRIPTION',
    'premiumEyebrow': 'PREMIUM',
    'premiumTagline': 'Divine grace, every day',
    'premiumPerMonthCaption': 'PER MONTH',
    'premiumTrialLeadPrefix': 'Start your 1-day FREE trial for ',
    'premiumRefundedBadge': 'REFUNDED INSTANTLY',
    'premiumFeatureWallpapers': 'Unlimited HD Wallpapers',
    'premiumFeatureRingtones': 'Devotional Ringtones',
    'premiumFeatureDaily': 'Daily New Content',
    'premiumCtaTrial': 'Start Free Trial',
    'premiumCtaSubscribe': 'Subscribe Now',
    'premiumReassuranceTrial':
        '₹2 verification, refunded instantly · Cancel anytime',
    'premiumReassurancePaid':
        'Secured by UPI Autopay · Cancel anytime in one tap',
    'premiumSelectedUpiApp': 'Selected UPI App',
    'upiPickerTitle': 'Pay using',
    'upiPickerLastUsed': 'Last used',
    'premiumTrialFinePrint': 'Then {price}/month via autopay. Cancel anytime.',
    'premiumSocialProof': '{name} in {city} just applied a live wallpaper 🙏',
  },
  'ta': {
    'appName': 'அருள்',
    'appTagline': 'தென்னிந்திய வால்பேப்பர்கள்',
    'feedLiveBadge': 'லைவ்',
    'feedEmptyTitle': 'இங்கு இன்னும் எதுவும் இல்லை',
    'feedEmptyBody': 'வேறு பிரிவை முயற்சிக்கவும்.',
    'feedErrorTitle': 'வால்பேப்பர்களை ஏற்ற முடியவில்லை',
    'feedErrorBody': 'இணைப்பைச் சரிபார்த்து மீண்டும் முயற்சிக்கவும்.',
    'errorGeneric': 'ஏதோ தவறு நடந்தது.',
    'signInCaption': 'தெய்வீகம் உங்கள் இல்லத்தில்',
    'signInGoogle': 'Google-இல் தொடர்க',
    'signInSubtitleIdle': 'கணக்கைத் தேர்வுசெய்க',
    'signInSubtitleExchanging': 'உள்நுழைகிறது…',
    'signInNudgeBackedOutQuick': 'முடியவில்லையா? தட்டவும்',
    'signInNudgeBackedOutSlow': 'தட்டி Google-ஐக் காக்கவும்',
    'signInNudgeNeverOpened': 'Google வரவில்லை, தட்டவும்',
    'signInNudgeAddAccount': 'உள்ள கணக்கைத் தேர்க',
    'signInNudgeReauth': 'மீண்டும் கணக்கைத் தேர்க',
    'signInNudgeActivityClosed': 'சாளரம் மூடியது, தட்டவும்',
    'signInNudgeNoProvider': 'Google Play புதுப்பிக்கவும்',
    'signInFixBackedOutSlow':
        'இந்தத் தொலைபேசியில் Google சற்று நேரம் எடுக்கிறது.',
    'signInFixAddAccount':
        'புதிய கணக்கு தேவையில்லை. இந்தத் தொலைபேசியில் ஏற்கனவே உள்ள எந்த Google கணக்கும் போதும்.',
    'signInFixReauth':
        'அந்தக் கணக்கை Google சரிபார்க்க முடியவில்லை. தொலைபேசி அமைப்புகளில் Google-இல் மீண்டும் உள்நுழையவும்.',
    'signInFixActivityClosed':
        'Google Play சேவைகள் உள்நுழைவுச் சாளரத்தை மூடிவிட்டன.',
    'signInLinkAccountSettings': 'கணக்கு அமைப்புகளைத் திறக்க',
    'signInLinkPlayServices': 'Google Play சேவைகளைப் பார்க்க',
    'signInLinkPlayStore': 'Play Store-இல் புதுப்பிக்க',
    'signInTermsLink': 'விதிமுறைகள்',
    'signInPrivacyLink': 'தனியுரிமை',
    'premiumTitle': 'பிரீமியம்',
    'premiumHeadline': 'அனைத்து வால்பேப்பர்களையும் திறக்கவும்',
    'premiumSub': 'உலாவுவது எப்போதும் இலவசம். பிரீமியம் அவற்றை உங்களுடையதாக்க.',
    'premiumBenefitApply':
        'எந்த வால்பேப்பரையும் பயன்படுத்தவும் — நிலையான அல்லது லைவ்',
    'premiumBenefitLive': 'முழுத் தரத்தில் லைவ் வீடியோ வால்பேப்பர்கள்',
    'premiumBenefitShare': 'நண்பர்கள் மற்றும் குடும்பத்துடன் பகிரவும்',
    'premiumBenefitNew': 'ஒவ்வொரு வாரமும் புதிய வால்பேப்பர்கள்',
    'premiumPrice': '₹199 / மாதம்',
    'premiumCta': 'இலவச சோதனையைத் தொடங்கு',
    'premiumTrialNote':
        'ஒரு கணக்குக்கு ஒரு இலவச சோதனை. எப்போது வேண்டுமானாலும் ரத்து செய்யலாம் — காலம் முடியும் வரை அணுகல் தொடரும்.',
    'premiumComingSoon': 'பணம் செலுத்தும் வசதி பின்னர் வரும்.',
    'settingsTitle': 'அமைப்புகள்',
    'settingsAppearance': 'தோற்றம்',
    'themeSystem': 'சிஸ்டம்',
    'themeLight': 'வெளிச்சம்',
    'themeDark': 'இருள்',
    'settingsContent': 'உள்ளடக்கம்',
    'settingsAbout': 'பற்றி',
    'settingsSupport': 'உதவி தேவை',
    'settingsPrivacy': 'தனியுரிமைக் கொள்கை',
    'settingsVersion': 'பதிப்பு',
    'uploadBody':
        'உங்கள் சொந்த வால்பேப்பரைப் பகிரவும். ஒவ்வொன்றையும் நாங்கள் பரிசீலிக்கிறோம்.',
    'uploadPickCategory': 'ஒரு பிரிவைத் தேர்ந்தெடுக்கவும்',
    'uploadPickFile': 'ஒரு கோப்பைத் தேர்ந்தெடுக்கவும்',
    'uploadSpecNote':
        'படங்கள்: 1080×1920. வீடியோக்கள்: 1024×1824, ஒலி இல்லாமல், 50 MB க்குள்.',
    'uploadComingSoon': 'பதிவேற்றம் பின்னர் வரும்.',
    'apply': 'பயன்படுத்து',
    'share': 'பகிர்',
    'applyTargetTitle': 'வால்பேப்பரை அமைக்க',
    'applyTargetHome': 'முகப்பு திரை',
    'applyTargetLock': 'பூட்டு திரை',
    'applyTargetBoth': 'முகப்பு மற்றும் பூட்டு திரை',
    'applied': 'வால்பேப்பர் அமைக்கப்பட்டது',
    'appliedLiveFallback':
        'இந்த போனில் லைவ் வால்பேப்பர்கள் இயங்காது, எனவே நிலையான படத்தை அமைத்தோம்.',
    'offlineBody':
        'இணைய இணைப்பு இல்லை. இணைப்பைச் சரிபார்த்து மீண்டும் முயற்சிக்கவும்.',
    'offlineTitle': 'இணையம் இல்லை',
    'offlineFeedBody': 'வால்பேப்பர்களைப் பார்க்க இணையத்தை இயக்கவும்.',
    'wallpaperShareCaption':
        'இது போன்ற மேலும் பக்தி வால்பேப்பர்கள் — நிலையானவை மற்றும் லைவ் — Arul-இல்:\n{link}',
    'referShareMessage':
        'தென்னிந்திய பக்தி வால்பேப்பர்களுக்காக நான் Arul பயன்படுத்தி வருகிறேன் — அம்மன், முருகன், பெருமாள், சிவன், அசையும் லைவ் வால்பேப்பர்களும் உண்டு. உங்களுக்கும் பிடிக்கும் என்று நினைத்தேன்.\n\n{link}',
    'tabWallpapers': 'வால்பேப்பர்கள்',
    'ringtonePreviewSemantic': 'முன்னோட்டம்',
    'ringtonePreviewUnavailable': 'முன்னோட்டம் இன்னும் கிடைக்கவில்லை',
    'ringtonesEmptyTitle': 'ரிங்டோன்கள் விரைவில் வருகின்றன',
    'ringtonesEmptyBody':
        'பக்தி ரிங்டோன்கள் வழியில் உள்ளன. சிறிது நேரம் கழித்து மீண்டும் பாருங்கள்.',
    'ringtonesErrorTitle': 'ரிங்டோன்களை ஏற்ற முடியவில்லை',
    'ringtoneSetSuccess':
        'ரிங்டோன் அமைக்கப்பட்டது. தெரியவில்லை என்றால், உங்கள் ஃபோனை மறுதொடக்கம் செய்யுங்கள்.',
    'ringtoneSetFailed':
        'ரிங்டோனை அமைக்க முடியவில்லை. மீண்டும் முயற்சிக்கவும்.',
    'ringtoneSetPreparing': 'தயாராகிறது…',
    'ringtoneSetDownloading': 'பதிவிறக்கம் ஆகிறது…',
    'ringtoneSetApplying': 'ரிங்டோன் அமைக்கப்படுகிறது…',
    'save': 'சேமி',
    'cancel': 'ரத்து',
    'errorGenericRetry': 'ஏதோ தவறு நடந்தது. மீண்டும் முயற்சிக்கவும்.',
    'settingsPremiumSubLocked': 'பயன்படுத்தவும், பகிரவும் திறக்க',
    'settingsPremiumSubTrial': 'இலவச சோதனையில் இருக்கிறீர்கள்',
    'settingsPremiumSubCancelled':
        'தானியங்கு புதுப்பித்தல் அணைக்கப்பட்டுள்ளது · அணுகல் தொடரும்',
    'settingsPremiumSubActive': 'நீங்கள் உறுப்பினர்',
    'settingsReferSub': '30 நாள் இலவச பிரீமியம் சம்பாதிக்கவும்',
    'settingsTellFriend': 'நண்பரிடம் சொல்லுங்கள்',
    'settingsTellFriendSub': 'Arul-ஐ ரசிக்கும் ஒருவருக்கு அனுப்புங்கள்',
    'settingsRemindersSubOn':
        'வாராந்திர மற்றும் பண்டிகை நினைவூட்டல்கள் இயக்கத்தில்',
    'settingsRemindersSubOff': 'பண்டிகை மற்றும் வாராந்திர நினைவூட்டல்கள்',
    'settingsLanguage': 'மொழி',
    'settingsTheme': 'தீம்',
    'settingsNeedHelp': 'உதவி தேவையா?',
    'settingsNeedHelpSub': 'சப்போர்ட்டைத் தொடர்பு கொள்ளவும்',
    'settingsUpload': 'உங்கள் உள்ளடக்கத்தைப் பதிவேற்றவும்',
    'settingsUploadSub': 'வால்பேப்பர் அல்லது ரிங்டோனைப் பகிரவும்',
    'settingsLogout': 'வெளியேறு',
    'settingsLogoutConfirmTitle': 'வெளியேறவா?',
    'settingsLogoutConfirmBody':
        'எப்போது வேண்டுமானாலும் Google மூலம் மீண்டும் உள்நுழையலாம்.',
    'settingsDeleteAccount': 'கணக்கை நீக்கு',
    'settingsDeleteConfirmTitle': 'கணக்கை நீக்கவா?',
    'settingsDeleteConfirmBody':
        'இது உங்கள் கணக்கு, விருப்பங்கள் மற்றும் வெகுமதிகளை நிரந்தரமாக நீக்கும்.',
    'settingsTerms': 'விதிமுறைகள் & நிபந்தனைகள்',
    'settingsRefund': 'பணத்திரும்பக் கொள்கை',
    'settingsSupportEmailPrompt': 'உங்கள் சிக்கல் அல்லது கருத்தை விவரிக்கவும்:',
    'settingsSupportEmailDetails':
        'கீழே உள்ள விவரங்கள் உங்கள் கோரிக்கையை விரைவாகத் தீர்க்க உதவும்:',
    'settingsNoEmailApp': 'மின்னஞ்சல் ஆப் இல்லை. {email} க்கு எழுதுங்கள்',
    'settingsEditNameTitle': 'உங்கள் பெயர்',
    'themeSystemDefault': 'சிஸ்டம் இயல்புநிலை',
    'themeSystemSub': 'சாதன அமைப்பைப் பின்பற்று',
    'themeLightSub': 'தந்தம் & பட்டு',
    'themeDarkSub': 'விளக்கொளி மெரூன்',
    'remindersToggleTitle': 'பக்தி நினைவூட்டல்கள்',
    'remindersToggleSub': 'வாராந்திர நாள், மற்றும் ஒவ்வொரு முக்கிய பண்டிகையும்',
    'remindersScheduleNote':
        'ஒவ்வொரு வாரமும் வெள்ளிக்கிழமை ஒரு நினைவூட்டல், மேலும் ஒவ்வொரு முக்கிய பண்டிகைக்கும் சில நாட்கள் முன்பு ஒன்று — பொங்கல், தீபாவளி, நவராத்திரி, சிவராத்திரி மற்றும் பிற. மாதத்திற்கு சுமார் இரண்டு.',
    'remindersPermissionToast':
        'Arul-க்கு அறிவிப்புகள் அணைக்கப்பட்டுள்ளன. நினைவூட்டல்கள் பெற உங்கள் ஃபோன் அமைப்புகளில் இயக்கவும்.',
    'remindersTimeLabel': 'நினைவூட்டல் நேரம்',
    'remindersComingUp': 'வரப்போவது',
    'remindersMonthJan': 'ஜன',
    'remindersMonthFeb': 'பிப்',
    'remindersMonthMar': 'மார்',
    'remindersMonthApr': 'ஏப்',
    'remindersMonthMay': 'மே',
    'remindersMonthJun': 'ஜூன்',
    'remindersMonthJul': 'ஜூலை',
    'remindersMonthAug': 'ஆக',
    'remindersMonthSep': 'செப்',
    'remindersMonthOct': 'அக்',
    'remindersMonthNov': 'நவ',
    'remindersMonthDec': 'டிச',
    'premiumBrandTitle': 'Arul Premium',
    'premiumScreenSubline': 'முழு சேகரிப்பு, உங்கள் திரையில் உயிர்ப்புடன்',
    'premiumPerkEvery': 'ஒவ்வொரு வால்பேப்பரும் — நிலையானது மற்றும் லைவ்',
    'premiumPerkApplyShare': 'வரம்பில்லாமல் பயன்படுத்தவும் பகிரவும்',
    'premiumPerkNew': 'ஒவ்வொரு வாரமும் புதிய வரவுகள்',
    'premiumPerMonth': '/ மாதம்',
    'premiumPlanNote': 'UPI ஆட்டோபே · எப்போதும் ரத்து செய்யலாம்',
    'premiumTrialPill': '1 நாள் இலவசம்',
    'premiumCtaPaid': 'பிரீமியம் பெறு',
    'premiumFootnoteTrial':
        '1 நாள் இலவசம், பிறகு மாதம் {price}. UPI ஆட்டோபே உங்கள் கணக்கை ₹2 மூலம் சரிபார்க்கும், உடனே திரும்பக் கிடைக்கும். உலாவுவது எப்போதும் இலவசம்.',
    'premiumFootnotePaid':
        'இன்று {price} வசூலிக்கப்படும், பிறகு UPI ஆட்டோபே மூலம் மாதந்தோறும் புதுப்பிக்கப்படும். எப்போதும் ரத்து செய்யலாம். உலாவுவது எப்போதும் இலவசம்.',
    'premiumComingSoonToast': 'பிரீமியம் விரைவில் வருகிறது.',
    'premiumWelcomeToast': 'Arul Premium-க்கு வரவேற்கிறோம்!',
    'premiumCelebrateTitle': 'நீங்கள் இணைந்துவிட்டீர்கள்',
    'premiumCelebrateBody':
        'Arul Premium செயலில் உள்ளது. இந்த வால்பேப்பர்களை ரசிக்கும் ஒருவர் தெரிகிறாரா? ஒன்றை அனுப்புங்கள்.',
    'premiumSheetPitch':
        'ஒவ்வொரு வால்பேப்பரும் — லைவ் மற்றும் நிலையானது. ஆறு பிரிவுகளிலும் தடையின்றி பயன்படுத்தவும் பகிரவும்.',
    'premiumKeepBrowsing': 'இலவசமாக உலாவுங்கள்',
    'referHeroTitle': 'நண்பருக்குப் பரிசு, உங்களுக்கு ஒரு மாதம்',
    'referHeroBody':
        'உங்கள் இணைப்பு மூலம் சந்தா செய்யும் ஒவ்வொரு நண்பருக்கும் 30 நாள் இலவச பிரீமியம்',
    'referRewardsLabel': 'சம்பாதித்த வெகுமதிகள்',
    'referRewardDays': '{days} நாட்கள்',
    'referStep1': 'உங்கள் இணைப்பை நண்பர்கள், குடும்பத்துடன் பகிரவும்',
    'referStep2': 'அவர்கள் Arul-ஐ நிறுவி பிரீமியம் சந்தா செய்கிறார்கள்',
    'referStep3': '30 நாள் இலவச பிரீமியம் உங்கள் கணக்கில் சேரும்',
    'referEmpty': 'இன்னும் பரிந்துரைகள் இல்லை — ஒரு பகிர்வில் முதல் நண்பர்',
    'referShareCta': 'Arul-ஐப் பகிர்',
    'referNotNow': 'இப்போது வேண்டாம்',
    'uploadScreenTitle': 'வால்பேப்பர் பதிவேற்றம்',
    'uploadPickZoneSub': 'போர்ட்ரெயிட், 1080×2400 அல்லது பெரியது',
    'uploadTitleLabel': 'தலைப்பு',
    'uploadTitleOptional': '(விருப்பம்)',
    'uploadCategoryLabel': 'பிரிவு',
    'uploadRightsCheckbox':
        'இந்த உள்ளடக்கத்தின் உரிமை என்னுடையது, அல்லது பகிர அனுமதி உள்ளது',
    'uploadFootnote':
        'அனுமதிக்கப்பட்ட வால்பேப்பர்கள் உங்கள் பெயருடன் ஃபீடில் தோன்றும்',
    'uploadRejectStatic': 'JPEG, PNG அல்லது WebP படத்தைத் தேர்ந்தெடுக்கவும்.',
    'uploadRejectLive': 'MP4 வீடியோவைத் தேர்ந்தெடுக்கவும்.',
    'uploadRejectAudio':
        'MP3, AAC அல்லது M4A ஒலிக் கோப்பைத் தேர்ந்தெடுக்கவும்.',
    'uploadKindLabel': 'எதைப் பகிர்கிறீர்கள்?',
    'uploadKindWallpaper': 'வால்பேப்பர்',
    'uploadKindRingtone': 'ரிங்டோன்',
    'uploadPickZoneTitleAudio': 'ஒலிக் கோப்பைத் தேர்ந்தெடுக்கவும்',
    'uploadPickZoneSubAudio': 'MP3, AAC அல்லது M4A',
    'uploadTitleHintRingtone': 'எ.கா. கந்த சஷ்டி கவசம்',
    'uploadFootnoteRingtone':
        'அனுமதிக்கப்பட்ட ரிங்டோன்கள் உங்கள் பெயருடன் ரிங்டோன்கள் தாவலில் தோன்றும்',
    'uploadShareMomentBodyRingtone':
        'உங்கள் ரிங்டோனை விரைவில் பரிசீலிக்கிறோம். அதுவரை — Arul-ஐ ரசிக்கும் ஒருவர் தெரிகிறாரா?',
    'uploadTooLarge': 'கோப்பு மிகப் பெரியது (அதிகபட்சம் {max}).',
    'uploadSuccessToast': 'பரிசீலனைக்கு அனுப்பப்பட்டது — நன்றி!',
    'uploadShareMomentTitle': 'நன்றி',
    'uploadShareMomentBody':
        'உங்கள் வால்பேப்பரை விரைவில் பரிசீலிக்கிறோம். அதுவரை — Arul-ஐ ரசிக்கும் ஒருவர் தெரிகிறாரா?',
    'uploadComingSoonToast': 'பதிவேற்றம் விரைவில் வருகிறது.',
    'premiumNavTitle': 'சந்தா',
    'premiumEyebrow': 'பிரீமியம்',
    'premiumTagline': 'தினமும் தெய்வ அருள்',
    'premiumPerMonthCaption': 'மாதம் ஒன்றுக்கு',
    'premiumTrialLeadPrefix': 'உங்கள் 1 நாள் இலவச சோதனையைத் தொடங்குங்கள் ',
    'premiumRefundedBadge': 'உடனே திரும்பக் கிடைக்கும்',
    'premiumFeatureWallpapers': 'வரம்பற்ற HD வால்பேப்பர்கள்',
    'premiumFeatureRingtones': 'பக்தி ரிங்டோன்கள்',
    'premiumFeatureDaily': 'தினமும் புதிய உள்ளடக்கம்',
    'premiumCtaTrial': 'இலவச சோதனையைத் தொடங்கு',
    'premiumCtaSubscribe': 'இப்போதே சந்தா செய்',
    'premiumReassuranceTrial':
        '₹2 சரிபார்ப்பு, உடனே திரும்பும் · எப்போதும் ரத்து செய்யலாம்',
    'premiumReassurancePaid': 'UPI ஆட்டோபே பாதுகாப்பு · ஒரே தட்டலில் ரத்து',
    'premiumSelectedUpiApp': 'UPI ஆப்',
    'upiPickerTitle': 'இதன் மூலம் செலுத்து',
    'upiPickerLastUsed': 'கடைசியாக பயன்படுத்தியது',
    'premiumTrialFinePrint':
        'பிறகு மாதம் {price} ஆட்டோபே மூலம். எப்போதும் ரத்து செய்யலாம்.',
    'premiumSocialProof': '{city}-ல் {name} லைவ் வால்பேப்பர் போட்டார் 🙏',
  },
  'te': {
    'appName': 'అరుళ్',
    'appTagline': 'దక్షిణ భారత వాల్‌పేపర్లు',
    'feedLiveBadge': 'లైవ్',
    'feedEmptyTitle': 'ఇక్కడ ఇంకా ఏమీ లేదు',
    'feedEmptyBody': 'వేరే విభాగాన్ని ప్రయత్నించండి.',
    'feedErrorTitle': 'వాల్‌పేపర్లను లోడ్ చేయలేకపోయాం',
    'feedErrorBody': 'మీ కనెక్షన్ చూసి మళ్లీ ప్రయత్నించండి.',
    'errorGeneric': 'ఏదో తప్పు జరిగింది.',
    'signInCaption': 'దైవత్వం మీ ఇంటిలో',
    'signInGoogle': 'Googleతో కొనసాగించండి',
    'signInSubtitleIdle': 'ప్రారంభించడానికి ఖాతాను ఎంచుకోండి',
    'signInSubtitleExchanging': 'సైన్ ఇన్ చేస్తోంది…',
    'signInNudgeBackedOutQuick': 'పూర్తి కాలేదా? మళ్లీ నొక్కండి',
    'signInNudgeBackedOutSlow': 'ఒకసారి నొక్కి Google కోసం ఆగండి',
    'signInNudgeNeverOpened': 'Google తెరవలేదు, మళ్లీ నొక్కండి',
    'signInNudgeAddAccount': 'ఈ ఫోన్‌లోని ఖాతాను వాడండి',
    'signInNudgeReauth': 'మళ్లీ నొక్కి ఖాతాను ఎంచుకోండి',
    'signInNudgeActivityClosed': 'సైన్ ఇన్ ముందే మూసింది, నొక్కండి',
    'signInNudgeNoProvider': 'Google Play సేవలను నవీకరించండి',
    'signInFixBackedOutSlow': 'ఈ ఫోన్‌లో Google కొంత సమయం తీసుకుంటోంది.',
    'signInFixAddAccount':
        'కొత్త ఖాతా అవసరం లేదు. ఈ ఫోన్‌లో ఇప్పటికే ఉన్న ఏ Google ఖాతా అయినా పని చేస్తుంది.',
    'signInFixReauth':
        'ఆ ఖాతాను Google ధృవీకరించలేకపోయింది. ఫోన్ సెట్టింగ్‌లలో Googleకి మళ్లీ సైన్ ఇన్ చేయండి.',
    'signInFixActivityClosed': 'Google Play సేవలు సైన్ ఇన్ విండోను మూసివేశాయి.',
    'signInLinkAccountSettings': 'ఖాతా సెట్టింగ్‌లు తెరవండి',
    'signInLinkPlayServices': 'Google Play సేవలను చూడండి',
    'signInLinkPlayStore': 'Play Storeలో నవీకరించండి',
    'signInTermsLink': 'నిబంధనలు',
    'signInPrivacyLink': 'గోప్యత',
    'premiumTitle': 'ప్రీమియం',
    'premiumHeadline': 'ప్రతి వాల్‌పేపర్‌ను అన్‌లాక్ చేయండి',
    'premiumSub':
        'బ్రౌజింగ్ ఎప్పుడూ ఉచితం. ప్రీమియం వాటిని మీవిగా చేసుకోవడానికి.',
    'premiumBenefitApply': 'ఏ వాల్‌పేపర్‌నైనా వర్తింపజేయండి — స్థిరం లేదా లైవ్',
    'premiumBenefitLive': 'పూర్తి నాణ్యతలో లైవ్ వీడియో వాల్‌పేపర్లు',
    'premiumBenefitShare': 'స్నేహితులు, కుటుంబంతో పంచుకోండి',
    'premiumBenefitNew': 'ప్రతి వారం కొత్త వాల్‌పేపర్లు',
    'premiumPrice': '₹199 / నెల',
    'premiumCta': 'ఉచిత ట్రయల్ ప్రారంభించు',
    'premiumTrialNote':
        'ఖాతాకు ఒక ఉచిత ట్రయల్. ఎప్పుడైనా రద్దు చేయవచ్చు — వ్యవధి ముగిసే వరకు యాక్సెస్ ఉంటుంది.',
    'premiumComingSoon': 'చెల్లింపులు తర్వాత వస్తాయి.',
    'settingsTitle': 'సెట్టింగ్‌లు',
    'settingsAppearance': 'రూపం',
    'themeSystem': 'సిస్టమ్',
    'themeLight': 'లైట్',
    'themeDark': 'డార్క్',
    'settingsContent': 'కంటెంట్',
    'settingsAbout': 'గురించి',
    'settingsSupport': 'సహాయం కావాలి',
    'settingsPrivacy': 'గోప్యతా విధానం',
    'settingsVersion': 'వెర్షన్',
    'uploadBody':
        'మీ సొంత వాల్‌పేపర్‌ను పంచుకోండి. ప్రతి సమర్పణను మేము సమీక్షిస్తాం.',
    'uploadPickCategory': 'ఒక విభాగాన్ని ఎంచుకోండి',
    'uploadPickFile': 'ఫైల్‌ను ఎంచుకోండి',
    'uploadSpecNote':
        'ఫోటోలు: 1080×1920. వీడియోలు: 1024×1824, ఆడియో లేకుండా, 50 MB లోపు.',
    'uploadComingSoon': 'అప్‌లోడ్‌లు తర్వాత వస్తాయి.',
    'apply': 'వర్తించు',
    'share': 'పంచుకోండి',
    'applyTargetTitle': 'వాల్‌పేపర్‌ను సెట్ చేయండి',
    'applyTargetHome': 'హోమ్ స్క్రీన్',
    'applyTargetLock': 'లాక్ స్క్రీన్',
    'applyTargetBoth': 'హోమ్ మరియు లాక్ స్క్రీన్',
    'applied': 'వాల్‌పేపర్ సెట్ చేయబడింది',
    'appliedLiveFallback':
        'ఈ ఫోన్‌లో లైవ్ వాల్‌పేపర్లకు సపోర్ట్ లేదు, అందుకే స్థిర చిత్రాన్ని సెట్ చేసాం.',
    'offlineBody':
        'మీరు ఆఫ్‌లైన్‌లో ఉన్నారు. మీ కనెక్షన్‌ను చూడండి మరియు మళ్లీ ప్రయత్నించండి.',
    'offlineTitle': 'ఇంటర్నెట్ లేదు',
    'offlineFeedBody': 'వాల్‌పేపర్లను చూడటానికి ఇంటర్నెట్‌ను ఆన్ చేయండి.',
    'wallpaperShareCaption':
        'ఇలాంటి మరిన్ని భక్తి వాల్‌పేపర్లు — స్థిరం మరియు లైవ్ — Arulలో:\n{link}',
    'referShareMessage':
        'దక్షిణ భారత భక్తి వాల్‌పేపర్ల కోసం నేను Arul వాడుతున్నాను — అమ్మన్, మురుగన్, పెరుమాళ్, శివన్, కదిలే లైవ్ వాల్‌పేపర్లు కూడా ఉన్నాయి. మీకూ నచ్చుతుందని అనిపించింది.\n\n{link}',
    'tabWallpapers': 'వాల్‌పేపర్లు',
    'ringtonePreviewSemantic': 'ప్రివ్యూ',
    'ringtonePreviewUnavailable': 'ప్రివ్యూ ఇంకా అందుబాటులో లేదు',
    'ringtonesEmptyTitle': 'రింగ్‌టోన్లు త్వరలో వస్తున్నాయి',
    'ringtonesEmptyBody':
        'భక్తి రింగ్‌టోన్లు దారిలో ఉన్నాయి. కాసేపటి తర్వాత మళ్లీ చూడండి.',
    'ringtonesErrorTitle': 'రింగ్‌టోన్లను లోడ్ చేయలేకపోయాం',
    'ringtoneSetSuccess':
        'రింగ్‌టోన్ సెట్ అయింది. కనిపించకపోతే, ఫోన్‌ను రీస్టార్ట్ చేయండి.',
    'ringtoneSetFailed': 'రింగ్‌టోన్ సెట్ చేయలేకపోయాం. మళ్లీ ప్రయత్నించండి.',
    'ringtoneSetPreparing': 'సిద్ధమవుతోంది…',
    'ringtoneSetDownloading': 'డౌన్‌లోడ్ అవుతోంది…',
    'ringtoneSetApplying': 'రింగ్‌టోన్ సెట్ అవుతోంది…',
    'save': 'సేవ్ చేయి',
    'cancel': 'రద్దు',
    'errorGenericRetry': 'ఏదో తప్పు జరిగింది. మళ్లీ ప్రయత్నించండి.',
    'settingsPremiumSubLocked': 'వర్తించడం, పంచుకోవడం అన్‌లాక్ చేయండి',
    'settingsPremiumSubTrial': 'మీరు ఉచిత ట్రయల్‌లో ఉన్నారు',
    'settingsPremiumSubCancelled': 'ఆటో రెన్యువల్ ఆఫ్ · యాక్సెస్ కొనసాగుతుంది',
    'settingsPremiumSubActive': 'మీరు సభ్యులు',
    'settingsReferSub': '30 రోజుల ఉచిత ప్రీమియం సంపాదించండి',
    'settingsTellFriend': 'స్నేహితుడికి చెప్పండి',
    'settingsTellFriendSub': 'Arul ఇష్టపడే వారికి పంపండి',
    'settingsRemindersSubOn': 'వారపు, పండుగ రిమైండర్లు ఆన్‌లో ఉన్నాయి',
    'settingsRemindersSubOff': 'పండుగ, వారపు రిమైండర్లు',
    'settingsLanguage': 'భాష',
    'settingsTheme': 'థీమ్',
    'settingsNeedHelp': 'సహాయం కావాలా?',
    'settingsNeedHelpSub': 'సపోర్ట్‌ను సంప్రదించండి',
    'settingsUpload': 'మీ కంటెంట్‌ను అప్‌లోడ్ చేయండి',
    'settingsUploadSub': 'వాల్‌పేపర్ లేదా రింగ్‌టోన్ పంచుకోండి',
    'settingsLogout': 'లాగ్ అవుట్',
    'settingsLogoutConfirmTitle': 'లాగ్ అవుట్ చేయాలా?',
    'settingsLogoutConfirmBody': 'ఎప్పుడైనా Googleతో తిరిగి సైన్ ఇన్ చేయవచ్చు.',
    'settingsDeleteAccount': 'ఖాతా తొలగించు',
    'settingsDeleteConfirmTitle': 'ఖాతా తొలగించాలా?',
    'settingsDeleteConfirmBody':
        'ఇది మీ ఖాతా, ఇష్టమైనవి, రివార్డులను శాశ్వతంగా తొలగిస్తుంది.',
    'settingsTerms': 'నిబంధనలు & షరతులు',
    'settingsRefund': 'రీఫండ్ విధానం',
    'settingsSupportEmailPrompt': 'మీ సమస్య లేదా అభిప్రాయాన్ని వివరించండి:',
    'settingsSupportEmailDetails':
        'కింది వివరాలు మీ అభ్యర్థనను వేగంగా పరిష్కరించడానికి సహాయపడతాయి:',
    'settingsNoEmailApp': 'ఇమెయిల్ యాప్ కనిపించలేదు. {email} కు రాయండి',
    'settingsEditNameTitle': 'మీ పేరు',
    'themeSystemDefault': 'సిస్టమ్ డిఫాల్ట్',
    'themeSystemSub': 'పరికరం సెట్టింగ్‌ను అనుసరించు',
    'themeLightSub': 'ఐవరీ & పట్టు',
    'themeDarkSub': 'దీపకాంతి మెరూన్',
    'remindersToggleTitle': 'భక్తి రిమైండర్లు',
    'remindersToggleSub': 'వారపు రోజు, ప్రతి ముఖ్య పండుగ',
    'remindersScheduleNote':
        'ప్రతి వారం శుక్రవారం ఒక రిమైండర్, ప్రతి ముఖ్య పండుగకు కొన్ని రోజుల ముందు ఒకటి — పొంగల్, దీపావళి, నవరాత్రి, శివరాత్రి, ఇతరాలు. నెలకు సుమారు రెండు.',
    'remindersPermissionToast':
        'Arulకు నోటిఫికేషన్లు ఆఫ్‌లో ఉన్నాయి. రిమైండర్లు రావాలంటే ఫోన్ సెట్టింగ్‌లలో ఆన్ చేయండి.',
    'remindersTimeLabel': 'రిమైండర్ సమయం',
    'remindersComingUp': 'రాబోయేవి',
    'remindersMonthJan': 'జన',
    'remindersMonthFeb': 'ఫిబ్ర',
    'remindersMonthMar': 'మార్చి',
    'remindersMonthApr': 'ఏప్రి',
    'remindersMonthMay': 'మే',
    'remindersMonthJun': 'జూన్',
    'remindersMonthJul': 'జులై',
    'remindersMonthAug': 'ఆగ',
    'remindersMonthSep': 'సెప్',
    'remindersMonthOct': 'అక్టో',
    'remindersMonthNov': 'నవం',
    'remindersMonthDec': 'డిసెం',
    'premiumBrandTitle': 'Arul Premium',
    'premiumScreenSubline': 'పూర్తి సేకరణ, మీ స్క్రీన్‌పై సజీవంగా',
    'premiumPerkEvery': 'ప్రతి వాల్‌పేపర్ — స్థిరం, లైవ్',
    'premiumPerkApplyShare': 'పరిమితి లేకుండా వర్తింపజేయండి, పంచుకోండి',
    'premiumPerkNew': 'ప్రతి వారం కొత్తవి',
    'premiumPerMonth': '/ నెల',
    'premiumPlanNote': 'UPI ఆటోపే · ఎప్పుడైనా రద్దు చేయవచ్చు',
    'premiumTrialPill': '1 రోజు ఉచితం',
    'premiumCtaPaid': 'ప్రీమియం పొందు',
    'premiumFootnoteTrial':
        '1 రోజు ఉచితం, తర్వాత నెలకు {price}. UPI ఆటోపే మీ ఖాతాను ₹2తో ధృవీకరిస్తుంది, వెంటనే వాపసు వస్తుంది. బ్రౌజింగ్ ఎప్పటికీ ఉచితం.',
    'premiumFootnotePaid':
        'ఈరోజు {price} వసూలు అవుతుంది, తర్వాత UPI ఆటోపే ద్వారా నెలనెలా రెన్యువల్ అవుతుంది. ఎప్పుడైనా రద్దు చేయవచ్చు. బ్రౌజింగ్ ఎప్పటికీ ఉచితం.',
    'premiumComingSoonToast': 'ప్రీమియం త్వరలో వస్తుంది.',
    'premiumWelcomeToast': 'Arul Premiumకు స్వాగతం!',
    'premiumCelebrateTitle': 'మీరు చేరారు',
    'premiumCelebrateBody':
        'Arul Premium యాక్టివ్‌గా ఉంది. ఈ వాల్‌పేపర్లు ఇష్టపడే వారు తెలుసా? ఒకటి పంపండి.',
    'premiumSheetPitch':
        'ప్రతి వాల్‌పేపర్ — లైవ్, స్థిరం. ఆరు విభాగాల్లోనూ స్వేచ్ఛగా వర్తింపజేయండి, పంచుకోండి.',
    'premiumKeepBrowsing': 'ఉచితంగా బ్రౌజ్ చేస్తూ ఉండండి',
    'referHeroTitle': 'స్నేహితుడికి బహుమతి, మీకు ఒక నెల',
    'referHeroBody':
        'మీ లింక్‌తో సబ్‌స్క్రైబ్ చేసే ప్రతి స్నేహితుడికీ 30 రోజుల ఉచిత ప్రీమియం',
    'referRewardsLabel': 'సంపాదించిన రివార్డులు',
    'referRewardDays': '{days} రోజులు',
    'referStep1': 'మీ లింక్‌ను స్నేహితులు, కుటుంబంతో పంచుకోండి',
    'referStep2': 'వారు Arul ఇన్‌స్టాల్ చేసి ప్రీమియం సబ్‌స్క్రైబ్ చేస్తారు',
    'referStep3': '30 రోజుల ఉచిత ప్రీమియం మీ ఖాతాలో చేరుతుంది',
    'referEmpty': 'ఇంకా రిఫరల్స్ లేవు — ఒక షేర్‌తో మొదటి స్నేహితుడు',
    'referShareCta': 'Arul పంచుకో',
    'referNotNow': 'ఇప్పుడు కాదు',
    'uploadScreenTitle': 'వాల్‌పేపర్ అప్‌లోడ్',
    'uploadPickZoneSub': 'పోర్ట్రెయిట్, 1080×2400 లేదా పెద్దది',
    'uploadTitleLabel': 'శీర్షిక',
    'uploadTitleOptional': '(ఐచ్ఛికం)',
    'uploadCategoryLabel': 'విభాగం',
    'uploadRightsCheckbox':
        'ఈ కంటెంట్‌పై హక్కులు నాకే ఉన్నాయి, లేదా పంచుకోవడానికి అనుమతి ఉంది',
    'uploadFootnote': 'ఆమోదించిన వాల్‌పేపర్లు మీ పేరుతో ఫీడ్‌లో కనిపిస్తాయి',
    'uploadRejectStatic': 'JPEG, PNG లేదా WebP ఫోటోను ఎంచుకోండి.',
    'uploadRejectLive': 'MP4 వీడియోను ఎంచుకోండి.',
    'uploadRejectAudio': 'MP3, AAC లేదా M4A ఆడియో ఫైల్‌ను ఎంచుకోండి.',
    'uploadKindLabel': 'మీరు దేన్ని పంచుకుంటున్నారు?',
    'uploadKindWallpaper': 'వాల్‌పేపర్',
    'uploadKindRingtone': 'రింగ్‌టోన్',
    'uploadPickZoneTitleAudio': 'ఆడియో ఫైల్‌ను ఎంచుకోండి',
    'uploadPickZoneSubAudio': 'MP3, AAC లేదా M4A',
    'uploadTitleHintRingtone': 'ఉదా. కంద షష్ఠి కవచం',
    'uploadFootnoteRingtone':
        'ఆమోదించిన రింగ్‌టోన్లు మీ పేరుతో రింగ్‌టోన్ల ట్యాబ్‌లో కనిపిస్తాయి',
    'uploadShareMomentBodyRingtone':
        'మీ రింగ్‌టోన్‌ను త్వరలో సమీక్షిస్తాం. అప్పటివరకు — Arul ఇష్టపడే వారు తెలుసా?',
    'uploadTooLarge': 'ఫైల్ చాలా పెద్దది (గరిష్ఠం {max}).',
    'uploadSuccessToast': 'సమీక్షకు పంపబడింది — ధన్యవాదాలు!',
    'uploadShareMomentTitle': 'ధన్యవాదాలు',
    'uploadShareMomentBody':
        'మీ వాల్‌పేపర్‌ను త్వరలో సమీక్షిస్తాం. అప్పటివరకు — Arul ఇష్టపడే వారు తెలుసా?',
    'uploadComingSoonToast': 'అప్‌లోడ్ త్వరలో వస్తుంది.',
    'premiumNavTitle': 'సబ్‌స్క్రిప్షన్',
    'premiumEyebrow': 'ప్రీమియం',
    'premiumTagline': 'ప్రతిరోజూ దైవ కృప',
    'premiumPerMonthCaption': 'నెలకు',
    'premiumTrialLeadPrefix': 'మీ 1 రోజు ఉచిత ట్రయల్ ప్రారంభించండి ',
    'premiumRefundedBadge': 'వెంటనే వాపసు',
    'premiumFeatureWallpapers': 'అపరిమిత HD వాల్‌పేపర్లు',
    'premiumFeatureRingtones': 'భక్తి రింగ్‌టోన్‌లు',
    'premiumFeatureDaily': 'ప్రతిరోజూ కొత్త కంటెంట్',
    'premiumCtaTrial': 'ఉచిత ట్రయల్ ప్రారంభించు',
    'premiumCtaSubscribe': 'ఇప్పుడే సబ్‌స్క్రైబ్ చేయండి',
    'premiumReassuranceTrial':
        '₹2 ధృవీకరణ, వెంటనే వాపసు · ఎప్పుడైనా రద్దు చేయవచ్చు',
    'premiumReassurancePaid': 'UPI ఆటోపే భద్రత · ఒకే ట్యాప్‌లో రద్దు',
    'premiumSelectedUpiApp': 'UPI యాప్',
    'upiPickerTitle': 'దీని ద్వారా చెల్లించండి',
    'upiPickerLastUsed': 'చివరిగా వాడినది',
    'premiumTrialFinePrint':
        'తర్వాత నెలకు {price} ఆటోపే ద్వారా. ఎప్పుడైనా రద్దు చేయవచ్చు.',
    'premiumSocialProof': '{city}-లో {name} లైవ్ వాల్‌పేపర్ పెట్టారు 🙏',
  },
  'kn': {
    'appName': 'ಅರುಳ್',
    'appTagline': 'ದಕ್ಷಿಣ ಭಾರತದ ವಾಲ್‌ಪೇಪರ್‌ಗಳು',
    'feedLiveBadge': 'ಲೈವ್',
    'feedEmptyTitle': 'ಇಲ್ಲಿ ಇನ್ನೂ ಏನೂ ಇಲ್ಲ',
    'feedEmptyBody': 'ಬೇರೆ ವಿಭಾಗವನ್ನು ಪ್ರಯತ್ನಿಸಿ.',
    'feedErrorTitle': 'ವಾಲ್‌ಪೇಪರ್‌ಗಳನ್ನು ಲೋಡ್ ಮಾಡಲಾಗಲಿಲ್ಲ',
    'feedErrorBody': 'ನಿಮ್ಮ ಸಂಪರ್ಕವನ್ನು ಪರಿಶೀಲಿಸಿ ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಿ.',
    'errorGeneric': 'ಏನೋ ತಪ್ಪಾಗಿದೆ.',
    'signInCaption': 'ದೈವಿಕತೆ ನಿಮ್ಮ ಮನೆಯಲ್ಲಿ',
    'signInGoogle': 'Google ನಲ್ಲಿ ಮುಂದುವರಿಸಿ',
    'signInSubtitleIdle': 'ಪ್ರಾರಂಭಿಸಲು ಖಾತೆಯನ್ನು ಆರಿಸಿ',
    'signInSubtitleExchanging': 'ಸೈನ್ ಇನ್ ಆಗುತ್ತಿದೆ…',
    'signInNudgeBackedOutQuick': 'ಆಗಲಿಲ್ಲವೇ? ಮತ್ತೆ ಟ್ಯಾಪ್ ಮಾಡಿ',
    'signInNudgeBackedOutSlow': 'ಟ್ಯಾಪ್ ಮಾಡಿ Google ಗಾಗಿ ಕಾಯಿರಿ',
    'signInNudgeNeverOpened': 'Google ತೆರೆಯಲಿಲ್ಲ, ಟ್ಯಾಪ್ ಮಾಡಿ',
    'signInNudgeAddAccount': 'ಈ ಫೋನ್‌ನ ಖಾತೆಯನ್ನು ಬಳಸಿ',
    'signInNudgeReauth': 'ಮತ್ತೆ ಟ್ಯಾಪ್ ಮಾಡಿ ಖಾತೆ ಆರಿಸಿ',
    'signInNudgeActivityClosed': 'ಬೇಗ ಮುಚ್ಚಿತು, ಮತ್ತೆ ಟ್ಯಾಪ್ ಮಾಡಿ',
    'signInNudgeNoProvider': 'Google Play ಸೇವೆಗಳನ್ನು ನವೀಕರಿಸಿ',
    'signInFixBackedOutSlow':
        'ಈ ಫೋನ್‌ನಲ್ಲಿ Google ಸ್ವಲ್ಪ ಸಮಯ ತೆಗೆದುಕೊಳ್ಳುತ್ತಿದೆ.',
    'signInFixAddAccount':
        'ಹೊಸ ಖಾತೆ ಬೇಡ. ಈ ಫೋನ್‌ನಲ್ಲಿ ಈಗಾಗಲೇ ಇರುವ ಯಾವುದೇ Google ಖಾತೆ ಸಾಕು.',
    'signInFixReauth':
        'ಆ ಖಾತೆಯನ್ನು Google ಪರಿಶೀಲಿಸಲಾಗಲಿಲ್ಲ. ಫೋನ್ ಸೆಟ್ಟಿಂಗ್‌ಗಳಲ್ಲಿ Google ಗೆ ಮತ್ತೆ ಸೈನ್ ಇನ್ ಮಾಡಿ.',
    'signInFixActivityClosed':
        'Google Play ಸೇವೆಗಳು ಸೈನ್ ಇನ್ ವಿಂಡೋವನ್ನು ಮುಚ್ಚಿದವು.',
    'signInLinkAccountSettings': 'ಖಾತೆ ಸೆಟ್ಟಿಂಗ್‌ಗಳನ್ನು ತೆರೆಯಿರಿ',
    'signInLinkPlayServices': 'Google Play ಸೇವೆಗಳನ್ನು ನೋಡಿ',
    'signInLinkPlayStore': 'Play Store ನಲ್ಲಿ ನವೀಕರಿಸಿ',
    'signInTermsLink': 'ನಿಯಮಗಳು',
    'signInPrivacyLink': 'ಗೌಪ್ಯತೆ',
    'premiumTitle': 'ಪ್ರೀಮಿಯಂ',
    'premiumHeadline': 'ಪ್ರತಿ ವಾಲ್‌ಪೇಪರ್ ಅನ್ನು ಅನ್‌ಲಾಕ್ ಮಾಡಿ',
    'premiumSub': 'ಬ್ರೌಸಿಂಗ್ ಯಾವಾಗಲೂ ಉಚಿತ. ಪ್ರೀಮಿಯಂ ಅವುಗಳನ್ನು ನಿಮ್ಮದಾಗಿಸಲು.',
    'premiumBenefitApply': 'ಯಾವುದೇ ವಾಲ್‌ಪೇಪರ್ ಅನ್ವಯಿಸಿ — ಸ್ಥಿರ ಅಥವಾ ಲೈವ್',
    'premiumBenefitLive': 'ಪೂರ್ಣ ಗುಣಮಟ್ಟದ ಲೈವ್ ವೀಡಿಯೊ ವಾಲ್‌ಪೇಪರ್‌ಗಳು',
    'premiumBenefitShare': 'ಸ್ನೇಹಿತರು ಮತ್ತು ಕುಟುಂಬದೊಂದಿಗೆ ಹಂಚಿಕೊಳ್ಳಿ',
    'premiumBenefitNew': 'ಪ್ರತಿ ವಾರ ಹೊಸ ವಾಲ್‌ಪೇಪರ್‌ಗಳು',
    'premiumPrice': '₹199 / ತಿಂಗಳು',
    'premiumCta': 'ಉಚಿತ ಟ್ರಯಲ್ ಪ್ರಾರಂಭಿಸಿ',
    'premiumTrialNote':
        'ಖಾತೆಗೆ ಒಂದು ಉಚಿತ ಟ್ರಯಲ್. ಯಾವಾಗ ಬೇಕಾದರೂ ರದ್ದುಗೊಳಿಸಿ — ಅವಧಿ ಮುಗಿಯುವವರೆಗೆ ಪ್ರವೇಶ ಇರುತ್ತದೆ.',
    'premiumComingSoon': 'ಪಾವತಿಗಳು ನಂತರ ಬರುತ್ತವೆ.',
    'settingsTitle': 'ಸೆಟ್ಟಿಂಗ್‌ಗಳು',
    'settingsAppearance': 'ಗೋಚರತೆ',
    'themeSystem': 'ಸಿಸ್ಟಂ',
    'themeLight': 'ಲೈಟ್',
    'themeDark': 'ಡಾರ್ಕ್',
    'settingsContent': 'ವಿಷಯ',
    'settingsAbout': 'ಕುರಿತು',
    'settingsSupport': 'ಸಹಾಯ ಬೇಕು',
    'settingsPrivacy': 'ಗೌಪ್ಯತಾ ನೀತಿ',
    'settingsVersion': 'ಆವೃತ್ತಿ',
    'uploadBody':
        'ನಿಮ್ಮ ಸ್ವಂತ ವಾಲ್‌ಪೇಪರ್ ಹಂಚಿಕೊಳ್ಳಿ. ಪ್ರತಿ ಸಲ್ಲಿಕೆಯನ್ನು ನಾವು ಪರಿಶೀಲಿಸುತ್ತೇವೆ.',
    'uploadPickCategory': 'ವಿಭಾಗವನ್ನು ಆಯ್ಕೆಮಾಡಿ',
    'uploadPickFile': 'ಫೈಲ್ ಆಯ್ಕೆಮಾಡಿ',
    'uploadSpecNote':
        'ಫೋಟೋಗಳು: 1080×1920. ವೀಡಿಯೊಗಳು: 1024×1824, ಆಡಿಯೊ ಇಲ್ಲದೆ, 50 MB ಒಳಗೆ.',
    'uploadComingSoon': 'ಅಪ್‌ಲೋಡ್‌ಗಳು ನಂತರ ಬರುತ್ತವೆ.',
    'apply': 'ಅನ್ವಯಿಸಿ',
    'share': 'ಹಂಚಿಕೊಳ್ಳಿ',
    'applyTargetTitle': 'ವಾಲ್‌ಪೇಪರ್ ಸೆಟ್ ಮಾಡಿ',
    'applyTargetHome': 'ಮುಖ್ಯ ಪರದೆ',
    'applyTargetLock': 'ಲಾಕ್ ಪರದೆ',
    'applyTargetBoth': 'ಮುಖ್ಯ ಮತ್ತು ಲಾಕ್ ಪರದೆ',
    'applied': 'ವಾಲ್‌ಪೇಪರ್ ಸೆಟ್ ಆಗಿದೆ',
    'appliedLiveFallback':
        'ಈ ಫೋನ್‌ನಲ್ಲಿ ಲೈವ್ ವಾಲ್‌ಪೇಪರ್‌ಗಳಿಗೆ ಬೆಂಬಲ ಇಲ್ಲ, ಆದ್ದರಿಂದ ಸ್ಥಿರ ಚಿತ್ರವನ್ನು ಹೊಂದಿಸಿದ್ದೇವೆ.',
    'offlineBody':
        'ನೀವು ಆಫ್‌ಲೈನ್‌ನಲ್ಲಿದ್ದೀರಿ. ನಿಮ್ಮ ಸಂಪರ್ಕವನ್ನು ಪರಿಶೀಲಿಸಿ ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಿ.',
    'offlineTitle': 'ಇಂಟರ್ನೆಟ್ ಇಲ್ಲ',
    'offlineFeedBody': 'ವಾಲ್‌ಪೇಪರ್‌ಗಳನ್ನು ನೋಡಲು ಇಂಟರ್ನೆಟ್ ಆನ್ ಮಾಡಿ.',
    'wallpaperShareCaption':
        'ಇಂತಹ ಇನ್ನಷ್ಟು ಭಕ್ತಿ ವಾಲ್‌ಪೇಪರ್‌ಗಳು — ಸ್ಥಿರ ಮತ್ತು ಲೈವ್ — Arul ನಲ್ಲಿ:\n{link}',
    'referShareMessage':
        'ದಕ್ಷಿಣ ಭಾರತೀಯ ಭಕ್ತಿ ವಾಲ್‌ಪೇಪರ್‌ಗಳಿಗಾಗಿ ನಾನು Arul ಬಳಸುತ್ತಿದ್ದೇನೆ — ಅಮ್ಮನ್, ಮುರುಗನ್, ಪೆರುಮಾಳ್, ಶಿವನ್, ಚಲಿಸುವ ಲೈವ್ ವಾಲ್‌ಪೇಪರ್‌ಗಳೂ ಇವೆ. ನಿಮಗೂ ಇಷ್ಟವಾಗುತ್ತದೆ ಅನಿಸಿತು.\n\n{link}',
    'tabWallpapers': 'ವಾಲ್‌ಪೇಪರ್‌ಗಳು',
    'ringtonePreviewSemantic': 'ಮುನ್ನೋಟ',
    'ringtonePreviewUnavailable': 'ಮುನ್ನೋಟ ಇನ್ನೂ ಲಭ್ಯವಿಲ್ಲ',
    'ringtonesEmptyTitle': 'ರಿಂಗ್‌ಟೋನ್‌ಗಳು ಶೀಘ್ರದಲ್ಲೇ ಬರುತ್ತಿವೆ',
    'ringtonesEmptyBody':
        'ಭಕ್ತಿ ರಿಂಗ್‌ಟೋನ್‌ಗಳು ದಾರಿಯಲ್ಲಿವೆ. ಸ್ವಲ್ಪ ಸಮಯದ ನಂತರ ಮತ್ತೆ ನೋಡಿ.',
    'ringtonesErrorTitle': 'ರಿಂಗ್‌ಟೋನ್‌ಗಳನ್ನು ಲೋಡ್ ಮಾಡಲಾಗಲಿಲ್ಲ',
    'ringtoneSetSuccess':
        'ರಿಂಗ್‌ಟೋನ್ ಹೊಂದಿಸಲಾಗಿದೆ. ಕಾಣದಿದ್ದರೆ, ಫೋನ್ ಅನ್ನು ಮರುಪ್ರಾರಂಭಿಸಿ.',
    'ringtoneSetFailed': 'ರಿಂಗ್‌ಟೋನ್ ಹೊಂದಿಸಲಾಗಲಿಲ್ಲ. ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಿ.',
    'ringtoneSetPreparing': 'ಸಿದ್ಧವಾಗುತ್ತಿದೆ…',
    'ringtoneSetDownloading': 'ಡೌನ್‌ಲೋಡ್ ಆಗುತ್ತಿದೆ…',
    'ringtoneSetApplying': 'ರಿಂಗ್‌ಟೋನ್ ಹೊಂದಿಸಲಾಗುತ್ತಿದೆ…',
    'save': 'ಉಳಿಸಿ',
    'cancel': 'ರದ್ದು',
    'errorGenericRetry': 'ಏನೋ ತಪ್ಪಾಗಿದೆ. ಮತ್ತೆ ಪ್ರಯತ್ನಿಸಿ.',
    'settingsPremiumSubLocked': 'ಅನ್ವಯಿಸಲು, ಹಂಚಿಕೊಳ್ಳಲು ಅನ್‌ಲಾಕ್ ಮಾಡಿ',
    'settingsPremiumSubTrial': 'ನೀವು ಉಚಿತ ಟ್ರಯಲ್‌ನಲ್ಲಿದ್ದೀರಿ',
    'settingsPremiumSubCancelled': 'ಸ್ವಯಂ ನವೀಕರಣ ಆಫ್ · ಪ್ರವೇಶ ಮುಂದುವರಿಯುತ್ತದೆ',
    'settingsPremiumSubActive': 'ನೀವು ಸದಸ್ಯರು',
    'settingsReferSub': '30 ದಿನ ಉಚಿತ ಪ್ರೀಮಿಯಂ ಗಳಿಸಿ',
    'settingsTellFriend': 'ಸ್ನೇಹಿತರಿಗೆ ತಿಳಿಸಿ',
    'settingsTellFriendSub': 'Arul ಇಷ್ಟಪಡುವವರಿಗೆ ಕಳುಹಿಸಿ',
    'settingsRemindersSubOn': 'ವಾರದ ಮತ್ತು ಹಬ್ಬದ ರಿಮೈಂಡರ್‌ಗಳು ಆನ್',
    'settingsRemindersSubOff': 'ಹಬ್ಬ ಮತ್ತು ವಾರದ ರಿಮೈಂಡರ್‌ಗಳು',
    'settingsLanguage': 'ಭಾಷೆ',
    'settingsTheme': 'ಥೀಮ್',
    'settingsNeedHelp': 'ಸಹಾಯ ಬೇಕೇ?',
    'settingsNeedHelpSub': 'ಬೆಂಬಲವನ್ನು ಸಂಪರ್ಕಿಸಿ',
    'settingsUpload': 'ನಿಮ್ಮ ವಿಷಯವನ್ನು ಅಪ್‌ಲೋಡ್ ಮಾಡಿ',
    'settingsUploadSub': 'ವಾಲ್‌ಪೇಪರ್ ಅಥವಾ ರಿಂಗ್‌ಟೋನ್ ಹಂಚಿಕೊಳ್ಳಿ',
    'settingsLogout': 'ಲಾಗ್ ಔಟ್',
    'settingsLogoutConfirmTitle': 'ಲಾಗ್ ಔಟ್ ಮಾಡುವುದೇ?',
    'settingsLogoutConfirmBody':
        'ಯಾವಾಗ ಬೇಕಾದರೂ Google ಮೂಲಕ ಮತ್ತೆ ಸೈನ್ ಇನ್ ಮಾಡಬಹುದು.',
    'settingsDeleteAccount': 'ಖಾತೆ ಅಳಿಸಿ',
    'settingsDeleteConfirmTitle': 'ಖಾತೆ ಅಳಿಸುವುದೇ?',
    'settingsDeleteConfirmBody':
        'ಇದು ನಿಮ್ಮ ಖಾತೆ, ಮೆಚ್ಚಿನವು ಮತ್ತು ರಿವಾರ್ಡ್‌ಗಳನ್ನು ಶಾಶ್ವತವಾಗಿ ಅಳಿಸುತ್ತದೆ.',
    'settingsTerms': 'ನಿಯಮಗಳು ಮತ್ತು ಷರತ್ತುಗಳು',
    'settingsRefund': 'ಮರುಪಾವತಿ ನೀತಿ',
    'settingsSupportEmailPrompt': 'ನಿಮ್ಮ ಸಮಸ್ಯೆ ಅಥವಾ ಅಭಿಪ್ರಾಯವನ್ನು ವಿವರಿಸಿ:',
    'settingsSupportEmailDetails':
        'ಕೆಳಗಿನ ವಿವರಗಳು ನಿಮ್ಮ ವಿನಂತಿಯನ್ನು ಬೇಗ ಪರಿಹರಿಸಲು ಸಹಾಯ ಮಾಡುತ್ತವೆ:',
    'settingsNoEmailApp': 'ಇಮೇಲ್ ಆ್ಯಪ್ ಸಿಗಲಿಲ್ಲ. {email} ಗೆ ಬರೆಯಿರಿ',
    'settingsEditNameTitle': 'ನಿಮ್ಮ ಹೆಸರು',
    'themeSystemDefault': 'ಸಿಸ್ಟಂ ಡೀಫಾಲ್ಟ್',
    'themeSystemSub': 'ಸಾಧನದ ಸೆಟ್ಟಿಂಗ್ ಅನುಸರಿಸಿ',
    'themeLightSub': 'ಐವರಿ & ರೇಷ್ಮೆ',
    'themeDarkSub': 'ದೀಪದ ಬೆಳಕಿನ ಮೆರೂನ್',
    'remindersToggleTitle': 'ಭಕ್ತಿ ರಿಮೈಂಡರ್‌ಗಳು',
    'remindersToggleSub': 'ವಾರದ ದಿನ, ಮತ್ತು ಪ್ರತಿ ಪ್ರಮುಖ ಹಬ್ಬ',
    'remindersScheduleNote':
        'ಪ್ರತಿ ವಾರ ಶುಕ್ರವಾರ ಒಂದು ರಿಮೈಂಡರ್, ಮತ್ತು ಪ್ರತಿ ಪ್ರಮುಖ ಹಬ್ಬಕ್ಕೆ ಕೆಲವು ದಿನ ಮೊದಲು ಒಂದು — ಪೊಂಗಲ್, ದೀಪಾವಳಿ, ನವರಾತ್ರಿ, ಶಿವರಾತ್ರಿ ಮತ್ತು ಇತರೆ. ತಿಂಗಳಿಗೆ ಸುಮಾರು ಎರಡು.',
    'remindersPermissionToast':
        'Arul ಗೆ ಅಧಿಸೂಚನೆಗಳು ಆಫ್ ಆಗಿವೆ. ರಿಮೈಂಡರ್‌ಗಳು ಬರಲು ಫೋನ್ ಸೆಟ್ಟಿಂಗ್‌ಗಳಲ್ಲಿ ಆನ್ ಮಾಡಿ.',
    'remindersTimeLabel': 'ರಿಮೈಂಡರ್ ಸಮಯ',
    'remindersComingUp': 'ಮುಂದೆ ಬರುವುದು',
    'remindersMonthJan': 'ಜನ',
    'remindersMonthFeb': 'ಫೆಬ್ರ',
    'remindersMonthMar': 'ಮಾರ್ಚ್',
    'remindersMonthApr': 'ಏಪ್ರಿ',
    'remindersMonthMay': 'ಮೇ',
    'remindersMonthJun': 'ಜೂನ್',
    'remindersMonthJul': 'ಜುಲೈ',
    'remindersMonthAug': 'ಆಗ',
    'remindersMonthSep': 'ಸೆಪ್',
    'remindersMonthOct': 'ಅಕ್ಟೋ',
    'remindersMonthNov': 'ನವೆಂ',
    'remindersMonthDec': 'ಡಿಸೆಂ',
    'premiumBrandTitle': 'Arul Premium',
    'premiumScreenSubline': 'ಪೂರ್ಣ ಸಂಗ್ರಹ, ನಿಮ್ಮ ಪರದೆಯ ಮೇಲೆ ಜೀವಂತ',
    'premiumPerkEvery': 'ಪ್ರತಿ ವಾಲ್‌ಪೇಪರ್ — ಸ್ಥಿರ ಮತ್ತು ಲೈವ್',
    'premiumPerkApplyShare': 'ಮಿತಿಯಿಲ್ಲದೆ ಅನ್ವಯಿಸಿ ಮತ್ತು ಹಂಚಿಕೊಳ್ಳಿ',
    'premiumPerkNew': 'ಪ್ರತಿ ವಾರ ಹೊಸ ಸೇರ್ಪಡೆಗಳು',
    'premiumPerMonth': '/ ತಿಂಗಳು',
    'premiumPlanNote': 'UPI ಆಟೋಪೇ · ಯಾವಾಗ ಬೇಕಾದರೂ ರದ್ದು',
    'premiumTrialPill': '1 ದಿನ ಉಚಿತ',
    'premiumCtaPaid': 'ಪ್ರೀಮಿಯಂ ಪಡೆಯಿರಿ',
    'premiumFootnoteTrial':
        '1 ದಿನ ಉಚಿತ, ನಂತರ ತಿಂಗಳಿಗೆ {price}. UPI ಆಟೋಪೇ ನಿಮ್ಮ ಖಾತೆಯನ್ನು ₹2 ನಿಂದ ಪರಿಶೀಲಿಸುತ್ತದೆ, ತಕ್ಷಣ ಮರಳುತ್ತದೆ. ಬ್ರೌಸಿಂಗ್ ಯಾವಾಗಲೂ ಉಚಿತ.',
    'premiumFootnotePaid':
        'ಇಂದು {price} ವಿಧಿಸಲಾಗುತ್ತದೆ, ನಂತರ UPI ಆಟೋಪೇ ಮೂಲಕ ಪ್ರತಿ ತಿಂಗಳು ನವೀಕರಣ. ಯಾವಾಗ ಬೇಕಾದರೂ ರದ್ದು ಮಾಡಿ. ಬ್ರೌಸಿಂಗ್ ಯಾವಾಗಲೂ ಉಚಿತ.',
    'premiumComingSoonToast': 'ಪ್ರೀಮಿಯಂ ಶೀಘ್ರದಲ್ಲೇ ಬರುತ್ತದೆ.',
    'premiumWelcomeToast': 'Arul Premium ಗೆ ಸ್ವಾಗತ!',
    'premiumCelebrateTitle': 'ನೀವು ಸೇರಿದ್ದೀರಿ',
    'premiumCelebrateBody':
        'Arul Premium ಸಕ್ರಿಯವಾಗಿದೆ. ಈ ವಾಲ್‌ಪೇಪರ್‌ಗಳನ್ನು ಇಷ್ಟಪಡುವವರು ಗೊತ್ತೇ? ಒಂದನ್ನು ಕಳುಹಿಸಿ.',
    'premiumSheetPitch':
        'ಪ್ರತಿ ವಾಲ್‌ಪೇಪರ್ — ಲೈವ್ ಮತ್ತು ಸ್ಥಿರ. ಆರೂ ವಿಭಾಗಗಳಲ್ಲಿ ಮುಕ್ತವಾಗಿ ಅನ್ವಯಿಸಿ, ಹಂಚಿಕೊಳ್ಳಿ.',
    'premiumKeepBrowsing': 'ಉಚಿತವಾಗಿ ಬ್ರೌಸ್ ಮಾಡುತ್ತಿರಿ',
    'referHeroTitle': 'ಸ್ನೇಹಿತರಿಗೆ ಉಡುಗೊರೆ, ನಿಮಗೆ ಒಂದು ತಿಂಗಳು',
    'referHeroBody':
        'ನಿಮ್ಮ ಲಿಂಕ್‌ನಿಂದ ಚಂದಾದಾರರಾಗುವ ಪ್ರತಿ ಸ್ನೇಹಿತರಿಗೂ 30 ದಿನ ಉಚಿತ ಪ್ರೀಮಿಯಂ',
    'referRewardsLabel': 'ಗಳಿಸಿದ ರಿವಾರ್ಡ್‌ಗಳು',
    'referRewardDays': '{days} ದಿನ',
    'referStep1': 'ನಿಮ್ಮ ಲಿಂಕ್ ಅನ್ನು ಸ್ನೇಹಿತರು, ಕುಟುಂಬದೊಂದಿಗೆ ಹಂಚಿಕೊಳ್ಳಿ',
    'referStep2': 'ಅವರು Arul ಇನ್‌ಸ್ಟಾಲ್ ಮಾಡಿ ಪ್ರೀಮಿಯಂ ಚಂದಾದಾರರಾಗುತ್ತಾರೆ',
    'referStep3': '30 ದಿನ ಉಚಿತ ಪ್ರೀಮಿಯಂ ನಿಮ್ಮ ಖಾತೆಗೆ ಸೇರುತ್ತದೆ',
    'referEmpty': 'ಇನ್ನೂ ರೆಫರಲ್‌ಗಳಿಲ್ಲ — ಒಂದು ಶೇರ್‌ನಲ್ಲಿ ಮೊದಲ ಸ್ನೇಹಿತ',
    'referShareCta': 'Arul ಹಂಚಿಕೊಳ್ಳಿ',
    'referNotNow': 'ಈಗ ಬೇಡ',
    'uploadScreenTitle': 'ವಾಲ್‌ಪೇಪರ್ ಅಪ್‌ಲೋಡ್',
    'uploadPickZoneSub': 'ಪೋರ್ಟ್ರೇಟ್, 1080×2400 ಅಥವಾ ದೊಡ್ಡದು',
    'uploadTitleLabel': 'ಶೀರ್ಷಿಕೆ',
    'uploadTitleOptional': '(ಐಚ್ಛಿಕ)',
    'uploadCategoryLabel': 'ವಿಭಾಗ',
    'uploadRightsCheckbox':
        'ಈ ವಿಷಯದ ಹಕ್ಕುಗಳು ನನ್ನವು, ಅಥವಾ ಹಂಚಿಕೊಳ್ಳಲು ಅನುಮತಿ ಇದೆ',
    'uploadFootnote':
        'ಅನುಮೋದಿತ ವಾಲ್‌ಪೇಪರ್‌ಗಳು ನಿಮ್ಮ ಹೆಸರಿನೊಂದಿಗೆ ಫೀಡ್‌ನಲ್ಲಿ ಕಾಣಿಸುತ್ತವೆ',
    'uploadRejectStatic': 'JPEG, PNG ಅಥವಾ WebP ಚಿತ್ರವನ್ನು ಆಯ್ಕೆಮಾಡಿ.',
    'uploadRejectLive': 'MP4 ವೀಡಿಯೊ ಆಯ್ಕೆಮಾಡಿ.',
    'uploadRejectAudio': 'MP3, AAC ಅಥವಾ M4A ಆಡಿಯೋ ಫೈಲ್ ಆಯ್ಕೆಮಾಡಿ.',
    'uploadKindLabel': 'ನೀವು ಏನನ್ನು ಹಂಚಿಕೊಳ್ಳುತ್ತಿದ್ದೀರಿ?',
    'uploadKindWallpaper': 'ವಾಲ್‌ಪೇಪರ್',
    'uploadKindRingtone': 'ರಿಂಗ್‌ಟೋನ್',
    'uploadPickZoneTitleAudio': 'ಆಡಿಯೋ ಫೈಲ್ ಆಯ್ಕೆಮಾಡಿ',
    'uploadPickZoneSubAudio': 'MP3, AAC ಅಥವಾ M4A',
    'uploadTitleHintRingtone': 'ಉದಾ. ಕಂದ ಷಷ್ಠಿ ಕವಚಂ',
    'uploadFootnoteRingtone':
        'ಅನುಮೋದಿತ ರಿಂಗ್‌ಟೋನ್‌ಗಳು ನಿಮ್ಮ ಹೆಸರಿನೊಂದಿಗೆ ರಿಂಗ್‌ಟೋನ್‌ಗಳ ಟ್ಯಾಬ್‌ನಲ್ಲಿ ಕಾಣಿಸುತ್ತವೆ',
    'uploadShareMomentBodyRingtone':
        'ನಿಮ್ಮ ರಿಂಗ್‌ಟೋನ್ ಅನ್ನು ಶೀಘ್ರ ಪರಿಶೀಲಿಸುತ್ತೇವೆ. ಅಷ್ಟರಲ್ಲಿ — Arul ಇಷ್ಟಪಡುವವರು ಗೊತ್ತೇ?',
    'uploadTooLarge': 'ಫೈಲ್ ತುಂಬಾ ದೊಡ್ಡದು (ಗರಿಷ್ಠ {max}).',
    'uploadSuccessToast': 'ಪರಿಶೀಲನೆಗೆ ಕಳುಹಿಸಲಾಗಿದೆ — ಧನ್ಯವಾದ!',
    'uploadShareMomentTitle': 'ಧನ್ಯವಾದ',
    'uploadShareMomentBody':
        'ನಿಮ್ಮ ವಾಲ್‌ಪೇಪರ್ ಅನ್ನು ಶೀಘ್ರ ಪರಿಶೀಲಿಸುತ್ತೇವೆ. ಅಷ್ಟರಲ್ಲಿ — Arul ಇಷ್ಟಪಡುವವರು ಗೊತ್ತೇ?',
    'uploadComingSoonToast': 'ಅಪ್‌ಲೋಡ್ ಶೀಘ್ರದಲ್ಲೇ ಬರುತ್ತದೆ.',
    'premiumNavTitle': 'ಚಂದಾದಾರಿಕೆ',
    'premiumEyebrow': 'ಪ್ರೀಮಿಯಂ',
    'premiumTagline': 'ಪ್ರತಿದಿನ ದೈವಿಕ ಕೃಪೆ',
    'premiumPerMonthCaption': 'ತಿಂಗಳಿಗೆ',
    'premiumTrialLeadPrefix': 'ನಿಮ್ಮ 1 ದಿನದ ಉಚಿತ ಟ್ರಯಲ್ ಪ್ರಾರಂಭಿಸಿ ',
    'premiumRefundedBadge': 'ತಕ್ಷಣ ಮರುಪಾವತಿ',
    'premiumFeatureWallpapers': 'ಅನಿಯಮಿತ HD ವಾಲ್‌ಪೇಪರ್‌ಗಳು',
    'premiumFeatureRingtones': 'ಭಕ್ತಿ ರಿಂಗ್‌ಟೋನ್‌ಗಳು',
    'premiumFeatureDaily': 'ಪ್ರತಿದಿನ ಹೊಸ ವಿಷಯ',
    'premiumCtaTrial': 'ಉಚಿತ ಟ್ರಯಲ್ ಪ್ರಾರಂಭಿಸಿ',
    'premiumCtaSubscribe': 'ಈಗಲೇ ಚಂದಾದಾರರಾಗಿ',
    'premiumReassuranceTrial':
        '₹2 ಪರಿಶೀಲನೆ, ತಕ್ಷಣ ಮರುಪಾವತಿ · ಯಾವಾಗ ಬೇಕಾದರೂ ರದ್ದುಗೊಳಿಸಿ',
    'premiumReassurancePaid': 'UPI ಆಟೋಪೇ ಸುರಕ್ಷತೆ · ಒಂದೇ ಟ್ಯಾಪ್‌ನಲ್ಲಿ ರದ್ದು',
    'premiumSelectedUpiApp': 'UPI ಆ್ಯಪ್',
    'upiPickerTitle': 'ಇದರ ಮೂಲಕ ಪಾವತಿಸಿ',
    'upiPickerLastUsed': 'ಕೊನೆಯದಾಗಿ ಬಳಸಿದ್ದು',
    'premiumTrialFinePrint':
        'ನಂತರ ತಿಂಗಳಿಗೆ {price} ಆಟೋಪೇ ಮೂಲಕ. ಯಾವಾಗ ಬೇಕಾದರೂ ರದ್ದುಗೊಳಿಸಿ.',
    'premiumSocialProof': '{city}-ನಲ್ಲಿ {name} ಲೈವ್ ವಾಲ್‌ಪೇಪರ್ ಹಾಕಿದರು 🙏',
  },
  'ml': {
    'appName': 'അരുൾ',
    'appTagline': 'ദക്ഷിണേന്ത്യൻ വാൾപേപ്പറുകൾ',
    'feedLiveBadge': 'ലൈവ്',
    'feedEmptyTitle': 'ഇവിടെ ഇതുവരെ ഒന്നുമില്ല',
    'feedEmptyBody': 'മറ്റൊരു വിഭാഗം പരീക്ഷിക്കുക.',
    'feedErrorTitle': 'വാൾപേപ്പറുകൾ ലോഡ് ചെയ്യാനായില്ല',
    'feedErrorBody': 'കണക്ഷൻ പരിശോധിച്ച് വീണ്ടും ശ്രമിക്കുക.',
    'errorGeneric': 'എന്തോ കുഴപ്പം സംഭവിച്ചു.',
    'signInCaption': 'ദൈവികത നിങ്ങളുടെ വീട്ടിൽ',
    'signInGoogle': 'Google വഴി തുടരുക',
    'signInSubtitleIdle': 'അക്കൗണ്ട് തിരഞ്ഞെടുക്കുക',
    'signInSubtitleExchanging': 'സൈൻ ഇൻ ചെയ്യുന്നു…',
    'signInNudgeBackedOutQuick': 'നടന്നില്ലേ? വീണ്ടും ടാപ്പ് ചെയ്യൂ',
    'signInNudgeBackedOutSlow': 'ഒറ്റ ടാപ്പ്, Google-നെ കാക്കുക',
    'signInNudgeNeverOpened': 'Google തുറന്നില്ല, ടാപ്പ് ചെയ്യൂ',
    'signInNudgeAddAccount': 'ഉള്ള അക്കൗണ്ട് എടുക്കുക',
    'signInNudgeReauth': 'അക്കൗണ്ട് വീണ്ടും എടുക്കുക',
    'signInNudgeActivityClosed': 'നേരത്തെ അടച്ചു, വീണ്ടും ടാപ്പ്',
    'signInNudgeNoProvider': 'Google Play അപ്ഡേറ്റ് ചെയ്യൂ',
    'signInFixBackedOutSlow': 'ഈ ഫോണിൽ Google അൽപ്പം സമയമെടുക്കുന്നു.',
    'signInFixAddAccount':
        'പുതിയ അക്കൗണ്ട് വേണ്ട. ഈ ഫോണിൽ ഇപ്പോൾ ഉള്ള ഏത് Google അക്കൗണ്ടും മതി.',
    'signInFixReauth':
        'ആ അക്കൗണ്ട് Google-ന് പരിശോധിക്കാനായില്ല. ഫോൺ ക്രമീകരണങ്ങളിൽ Google-ൽ വീണ്ടും സൈൻ ഇൻ ചെയ്യുക.',
    'signInFixActivityClosed': 'Google Play സേവനങ്ങൾ സൈൻ ഇൻ വിൻഡോ അടച്ചു.',
    'signInLinkAccountSettings': 'അക്കൗണ്ട് ക്രമീകരണം തുറക്കുക',
    'signInLinkPlayServices': 'Google Play സേവനങ്ങൾ നോക്കുക',
    'signInLinkPlayStore': 'Play Store-ൽ അപ്ഡേറ്റ് ചെയ്യുക',
    'signInTermsLink': 'നിബന്ധനകൾ',
    'signInPrivacyLink': 'സ്വകാര്യത',
    'premiumTitle': 'പ്രീമിയം',
    'premiumHeadline': 'എല്ലാ വാൾപേപ്പറുകളും അൺലോക്ക് ചെയ്യുക',
    'premiumSub': 'ബ്രൗസിംഗ് എപ്പോഴും സൗജന്യം. പ്രീമിയം അവ നിങ്ങളുടേതാക്കാൻ.',
    'premiumBenefitApply':
        'ഏത് വാൾപേപ്പറും പ്രയോഗിക്കുക — സ്റ്റാറ്റിക് അല്ലെങ്കിൽ ലൈവ്',
    'premiumBenefitLive': 'മുഴുവൻ നിലവാരത്തിൽ ലൈവ് വീഡിയോ വാൾപേപ്പറുകൾ',
    'premiumBenefitShare': 'സുഹൃത്തുക്കളുമായും കുടുംബവുമായും പങ്കിടുക',
    'premiumBenefitNew': 'എല്ലാ ആഴ്ചയും പുതിയ വാൾപേപ്പറുകൾ',
    'premiumPrice': '₹199 / മാസം',
    'premiumCta': 'സൗജന്യ ട്രയൽ ആരംഭിക്കുക',
    'premiumTrialNote':
        'ഒരു അക്കൗണ്ടിന് ഒരു സൗജന്യ ട്രയൽ. എപ്പോൾ വേണമെങ്കിലും റദ്ദാക്കാം — കാലാവധി തീരും വരെ ആക്‌സസ് ഉണ്ടാകും.',
    'premiumComingSoon': 'പേയ്‌മെന്റുകൾ പിന്നീട് വരും.',
    'settingsTitle': 'ക്രമീകരണങ്ങൾ',
    'settingsAppearance': 'രൂപം',
    'themeSystem': 'സിസ്റ്റം',
    'themeLight': 'ലൈറ്റ്',
    'themeDark': 'ഡാർക്ക്',
    'settingsContent': 'ഉള്ളടക്കം',
    'settingsAbout': 'ആപ്പിനെക്കുറിച്ച്',
    'settingsSupport': 'സഹായം വേണം',
    'settingsPrivacy': 'സ്വകാര്യതാ നയം',
    'settingsVersion': 'പതിപ്പ്',
    'uploadBody':
        'നിങ്ങളുടെ സ്വന്തം വാൾപേപ്പർ പങ്കിടുക. ഓരോ സമർപ്പണവും ഞങ്ങൾ പരിശോധിക്കുന്നു.',
    'uploadPickCategory': 'ഒരു വിഭാഗം തിരഞ്ഞെടുക്കുക',
    'uploadPickFile': 'ഒരു ഫയൽ തിരഞ്ഞെടുക്കുക',
    'uploadSpecNote':
        'ഫോട്ടോകൾ: 1080×1920. വീഡിയോകൾ: 1024×1824, ഓഡിയോ ഇല്ലാതെ, 50 MB-ൽ താഴെ.',
    'uploadComingSoon': 'അപ്‌ലോഡുകൾ പിന്നീട് വരും.',
    'apply': 'പ്രയോഗിക്കുക',
    'share': 'പങ്കിടുക',
    'applyTargetTitle': 'വാൾപേപ്പർ സെറ്റ് ചെയ്യുക',
    'applyTargetHome': 'ഹോം സ്ക്രീൻ',
    'applyTargetLock': 'ലോക് സ്ക്രീൻ',
    'applyTargetBoth': 'ഹോം, ലോക് സ്ക്രീൻ',
    'applied': 'വാൾപേപ്പർ സെറ്റ് ചെയ്തു',
    'appliedLiveFallback':
        'ഈ ഫോൺ ലൈവ് വാൾപേപ്പർ പിന്തുണയ്ക്കുന്നില്ല, അതിനാൽ ഒരു നിശ്ചല ചിത്രം സെറ്റ് ചെയ്തു.',
    'offlineBody':
        'നിങ്ങൾ ഓഫ്‌ലൈനിലാണ്. കണക്ഷൻ പരിശോധിച്ച് വീണ്ടും ശ്രമിക്കുക.',
    'offlineTitle': 'ഇന്റർനെറ്റ് ഇല്ല',
    'offlineFeedBody': 'വാൾപേപ്പറുകൾ കാണാൻ ഇന്റർനെറ്റ് ഓണാക്കുക.',
    'wallpaperShareCaption':
        'ഇതുപോലുള്ള കൂടുതൽ ഭക്തി വാൾപേപ്പറുകൾ — സ്റ്റാറ്റിക്കും ലൈവും — Arul-ൽ:\n{link}',
    'referShareMessage':
        'ദക്ഷിണേന്ത്യൻ ഭക്തി വാൾപേപ്പറുകൾക്കായി ഞാൻ Arul ഉപയോഗിക്കുന്നു — അമ്മൻ, മുരുകൻ, പെരുമാൾ, ശിവൻ, ചലിക്കുന്ന ലൈവ് വാൾപേപ്പറുകളും ഉണ്ട്. നിങ്ങൾക്കും ഇഷ്ടപ്പെടുമെന്ന് തോന്നി.\n\n{link}',
    'tabWallpapers': 'വാൾപേപ്പറുകൾ',
    'ringtonePreviewSemantic': 'പ്രിവ്യൂ',
    'ringtonePreviewUnavailable': 'പ്രിവ്യൂ ഇപ്പോൾ ലഭ്യമല്ല',
    'ringtonesEmptyTitle': 'റിംഗ്ടോണുകൾ ഉടൻ വരുന്നു',
    'ringtonesEmptyBody':
        'ഭക്തി റിംഗ്ടോണുകൾ വഴിയിലുണ്ട്. അൽപസമയത്തിനു ശേഷം വീണ്ടും നോക്കുക.',
    'ringtonesErrorTitle': 'റിംഗ്ടോണുകൾ ലോഡ് ചെയ്യാനായില്ല',
    'ringtoneSetSuccess':
        'റിംഗ്ടോൺ സെറ്റ് ചെയ്തു. കാണുന്നില്ലെങ്കിൽ ഫോൺ റീസ്റ്റാർട്ട് ചെയ്യുക.',
    'ringtoneSetFailed': 'റിംഗ്ടോൺ സെറ്റ് ചെയ്യാനായില്ല. വീണ്ടും ശ്രമിക്കുക.',
    'ringtoneSetPreparing': 'തയ്യാറാകുന്നു…',
    'ringtoneSetDownloading': 'ഡൗൺലോഡ് ചെയ്യുന്നു…',
    'ringtoneSetApplying': 'റിംഗ്ടോൺ സെറ്റ് ചെയ്യുന്നു…',
    'save': 'സേവ് ചെയ്യുക',
    'cancel': 'റദ്ദാക്കുക',
    'errorGenericRetry': 'എന്തോ കുഴപ്പം സംഭവിച്ചു. വീണ്ടും ശ്രമിക്കുക.',
    'settingsPremiumSubLocked': 'പ്രയോഗിക്കാനും പങ്കിടാനും അൺലോക്ക് ചെയ്യുക',
    'settingsPremiumSubTrial': 'നിങ്ങൾ സൗജന്യ ട്രയലിലാണ്',
    'settingsPremiumSubCancelled': 'ഓട്ടോ റിന്യൂവൽ ഓഫ് · ആക്‌സസ് തുടരും',
    'settingsPremiumSubActive': 'നിങ്ങൾ അംഗമാണ്',
    'settingsReferSub': '30 ദിവസം സൗജന്യ പ്രീമിയം നേടുക',
    'settingsTellFriend': 'ഒരു സുഹൃത്തിനോട് പറയൂ',
    'settingsTellFriendSub': 'Arul ഇഷ്ടപ്പെടുന്ന ഒരാൾക്ക് അയക്കൂ',
    'settingsRemindersSubOn': 'പ്രതിവാര, ഉത്സവ റിമൈൻഡറുകൾ ഓണാണ്',
    'settingsRemindersSubOff': 'ഉത്സവ, പ്രതിവാര റിമൈൻഡറുകൾ',
    'settingsLanguage': 'ഭാഷ',
    'settingsTheme': 'തീം',
    'settingsNeedHelp': 'സഹായം വേണോ?',
    'settingsNeedHelpSub': 'സപ്പോർട്ടിനെ ബന്ധപ്പെടുക',
    'settingsUpload': 'നിങ്ങളുടെ ഉള്ളടക്കം അപ്‌ലോഡ് ചെയ്യുക',
    'settingsUploadSub': 'വാൾപേപ്പറോ റിംഗ്ടോണോ പങ്കിടുക',
    'settingsLogout': 'ലോഗ് ഔട്ട്',
    'settingsLogoutConfirmTitle': 'ലോഗ് ഔട്ട് ചെയ്യണോ?',
    'settingsLogoutConfirmBody':
        'എപ്പോൾ വേണമെങ്കിലും Google ഉപയോഗിച്ച് വീണ്ടും സൈൻ ഇൻ ചെയ്യാം.',
    'settingsDeleteAccount': 'അക്കൗണ്ട് ഇല്ലാതാക്കുക',
    'settingsDeleteConfirmTitle': 'അക്കൗണ്ട് ഇല്ലാതാക്കണോ?',
    'settingsDeleteConfirmBody':
        'ഇത് നിങ്ങളുടെ അക്കൗണ്ട്, പ്രിയപ്പെട്ടവ, റിവാർഡുകൾ എന്നിവ എന്നന്നേക്കുമായി നീക്കും.',
    'settingsTerms': 'നിബന്ധനകളും വ്യവസ്ഥകളും',
    'settingsRefund': 'റീഫണ്ട് നയം',
    'settingsSupportEmailPrompt': 'നിങ്ങളുടെ പ്രശ്നമോ അഭിപ്രായമോ വിവരിക്കുക:',
    'settingsSupportEmailDetails':
        'താഴെയുള്ള വിവരങ്ങൾ നിങ്ങളുടെ അഭ്യർത്ഥന വേഗത്തിൽ പരിഹരിക്കാൻ സഹായിക്കും:',
    'settingsNoEmailApp':
        'ഇമെയിൽ ആപ്പ് കണ്ടില്ല. {email} എന്ന വിലാസത്തിൽ എഴുതുക',
    'settingsEditNameTitle': 'നിങ്ങളുടെ പേര്',
    'themeSystemDefault': 'സിസ്റ്റം ഡിഫോൾട്ട്',
    'themeSystemSub': 'ഉപകരണ ക്രമീകരണം പിന്തുടരുക',
    'themeLightSub': 'ഐവറി & പട്ട്',
    'themeDarkSub': 'വിളക്കുവെളിച്ചത്തിലെ മെറൂൺ',
    'remindersToggleTitle': 'ഭക്തി റിമൈൻഡറുകൾ',
    'remindersToggleSub': 'പ്രതിവാര ദിവസവും എല്ലാ പ്രധാന ഉത്സവവും',
    'remindersScheduleNote':
        'എല്ലാ ആഴ്ചയും വെള്ളിയാഴ്ച ഒരു റിമൈൻഡർ, ഓരോ പ്രധാന ഉത്സവത്തിനും കുറച്ചു ദിവസം മുൻപ് ഒന്ന് — പൊങ്കൽ, ദീപാവലി, നവരാത്രി, ശിവരാത്രി തുടങ്ങിയവ. മാസത്തിൽ ഏകദേശം രണ്ട്.',
    'remindersPermissionToast':
        'Arul-ന് അറിയിപ്പുകൾ ഓഫാണ്. റിമൈൻഡറുകൾ കിട്ടാൻ ഫോൺ ക്രമീകരണങ്ങളിൽ ഓണാക്കുക.',
    'remindersTimeLabel': 'റിമൈൻഡർ സമയം',
    'remindersComingUp': 'വരാനിരിക്കുന്നത്',
    'remindersMonthJan': 'ജനു',
    'remindersMonthFeb': 'ഫെബ്രു',
    'remindersMonthMar': 'മാർ',
    'remindersMonthApr': 'ഏപ്രി',
    'remindersMonthMay': 'മേയ്',
    'remindersMonthJun': 'ജൂൺ',
    'remindersMonthJul': 'ജൂലൈ',
    'remindersMonthAug': 'ഓഗ',
    'remindersMonthSep': 'സെപ്',
    'remindersMonthOct': 'ഒക്ടോ',
    'remindersMonthNov': 'നവം',
    'remindersMonthDec': 'ഡിസം',
    'premiumBrandTitle': 'Arul Premium',
    'premiumScreenSubline': 'മുഴുവൻ ശേഖരം, നിങ്ങളുടെ സ്ക്രീനിൽ ജീവനോടെ',
    'premiumPerkEvery': 'എല്ലാ വാൾപേപ്പറും — സ്റ്റാറ്റിക്കും ലൈവും',
    'premiumPerkApplyShare': 'പരിധിയില്ലാതെ പ്രയോഗിക്കുക, പങ്കിടുക',
    'premiumPerkNew': 'എല്ലാ ആഴ്ചയും പുതിയവ',
    'premiumPerMonth': '/ മാസം',
    'premiumPlanNote': 'UPI ഓട്ടോപേ · എപ്പോൾ വേണമെങ്കിലും റദ്ദാക്കാം',
    'premiumTrialPill': '1 ദിവസം സൗജന്യം',
    'premiumCtaPaid': 'പ്രീമിയം എടുക്കൂ',
    'premiumFootnoteTrial':
        '1 ദിവസം സൗജന്യം, പിന്നെ മാസം {price}. UPI ഓട്ടോപേ നിങ്ങളുടെ അക്കൗണ്ട് ₹2 ഉപയോഗിച്ച് പരിശോധിക്കും, ഉടൻ തിരികെ കിട്ടും. ബ്രൗസിംഗ് എപ്പോഴും സൗജന്യം.',
    'premiumFootnotePaid':
        'ഇന്ന് {price} ഈടാക്കും, പിന്നീട് UPI ഓട്ടോപേ വഴി എല്ലാ മാസവും പുതുക്കും. എപ്പോൾ വേണമെങ്കിലും റദ്ദാക്കാം. ബ്രൗസിംഗ് എപ്പോഴും സൗജന്യം.',
    'premiumComingSoonToast': 'പ്രീമിയം ഉടൻ വരുന്നു.',
    'premiumWelcomeToast': 'Arul Premium-ലേക്ക് സ്വാഗതം!',
    'premiumCelebrateTitle': 'നിങ്ങൾ ചേർന്നു',
    'premiumCelebrateBody':
        'Arul Premium സജീവമാണ്. ഈ വാൾപേപ്പറുകൾ ഇഷ്ടപ്പെടുന്ന ആരെയെങ്കിലും അറിയാമോ? ഒന്ന് അയക്കൂ.',
    'premiumSheetPitch':
        'എല്ലാ വാൾപേപ്പറും — ലൈവും സ്റ്റാറ്റിക്കും. ആറു വിഭാഗങ്ങളിലും സ്വതന്ത്രമായി പ്രയോഗിക്കുക, പങ്കിടുക.',
    'premiumKeepBrowsing': 'സൗജന്യമായി ബ്രൗസ് ചെയ്യൂ',
    'referHeroTitle': 'സുഹൃത്തിന് സമ്മാനം, നിങ്ങൾക്ക് ഒരു മാസം',
    'referHeroBody':
        'നിങ്ങളുടെ ലിങ്ക് വഴി സബ്‌സ്‌ക്രൈബ് ചെയ്യുന്ന ഓരോ സുഹൃത്തിനും 30 ദിവസം സൗജന്യ പ്രീമിയം',
    'referRewardsLabel': 'നേടിയ റിവാർഡുകൾ',
    'referRewardDays': '{days} ദിവസം',
    'referStep1': 'നിങ്ങളുടെ ലിങ്ക് സുഹൃത്തുക്കൾക്കും കുടുംബത്തിനും പങ്കിടുക',
    'referStep2': 'അവർ Arul ഇൻസ്റ്റാൾ ചെയ്ത് പ്രീമിയം സബ്‌സ്‌ക്രൈബ് ചെയ്യുന്നു',
    'referStep3': '30 ദിവസം സൗജന്യ പ്രീമിയം നിങ്ങളുടെ അക്കൗണ്ടിൽ എത്തും',
    'referEmpty': 'ഇതുവരെ റഫറലുകളില്ല — ഒരു ഷെയറിൽ ആദ്യ സുഹൃത്ത്',
    'referShareCta': 'Arul പങ്കിടുക',
    'referNotNow': 'ഇപ്പോൾ വേണ്ട',
    'uploadScreenTitle': 'വാൾപേപ്പർ അപ്‌ലോഡ്',
    'uploadPickZoneSub': 'പോർട്രെയ്റ്റ്, 1080×2400 അല്ലെങ്കിൽ വലുത്',
    'uploadTitleLabel': 'തലക്കെട്ട്',
    'uploadTitleOptional': '(നിർബന്ധമല്ല)',
    'uploadCategoryLabel': 'വിഭാഗം',
    'uploadRightsCheckbox':
        'ഈ ഉള്ളടക്കത്തിന്റെ അവകാശം എനിക്കാണ്, അല്ലെങ്കിൽ പങ്കിടാൻ അനുമതിയുണ്ട്',
    'uploadFootnote': 'അനുവദിച്ച വാൾപേപ്പറുകൾ നിങ്ങളുടെ പേരോടെ ഫീഡിൽ വരും',
    'uploadRejectStatic': 'JPEG, PNG അല്ലെങ്കിൽ WebP ചിത്രം തിരഞ്ഞെടുക്കുക.',
    'uploadRejectLive': 'MP4 വീഡിയോ തിരഞ്ഞെടുക്കുക.',
    'uploadRejectAudio': 'MP3, AAC അല്ലെങ്കിൽ M4A ഓഡിയോ ഫയൽ തിരഞ്ഞെടുക്കുക.',
    'uploadKindLabel': 'നിങ്ങൾ എന്താണ് പങ്കിടുന്നത്?',
    'uploadKindWallpaper': 'വാൾപേപ്പർ',
    'uploadKindRingtone': 'റിംഗ്ടോൺ',
    'uploadPickZoneTitleAudio': 'ഓഡിയോ ഫയൽ തിരഞ്ഞെടുക്കുക',
    'uploadPickZoneSubAudio': 'MP3, AAC അല്ലെങ്കിൽ M4A',
    'uploadTitleHintRingtone': 'ഉദാ. കന്ദ ഷഷ്ഠി കവചം',
    'uploadFootnoteRingtone':
        'അനുവദിച്ച റിംഗ്ടോണുകൾ നിങ്ങളുടെ പേരോടെ റിംഗ്ടോൺ ടാബിൽ വരും',
    'uploadShareMomentBodyRingtone':
        'നിങ്ങളുടെ റിംഗ്ടോൺ ഉടൻ പരിശോധിക്കും. അതുവരെ — Arul ഇഷ്ടപ്പെടുന്ന ആരെയെങ്കിലും അറിയാമോ?',
    'uploadTooLarge': 'ഫയൽ വളരെ വലുതാണ് (പരമാവധി {max}).',
    'uploadSuccessToast': 'പരിശോധനയ്ക്ക് അയച്ചു — നന്ദി!',
    'uploadShareMomentTitle': 'നന്ദി',
    'uploadShareMomentBody':
        'നിങ്ങളുടെ വാൾപേപ്പർ ഉടൻ പരിശോധിക്കും. അതുവരെ — Arul ഇഷ്ടപ്പെടുന്ന ആരെയെങ്കിലും അറിയാമോ?',
    'uploadComingSoonToast': 'അപ്‌ലോഡ് ഉടൻ വരുന്നു.',
    'premiumNavTitle': 'സബ്‌സ്ക്രിപ്ഷൻ',
    'premiumEyebrow': 'പ്രീമിയം',
    'premiumTagline': 'എന്നും ദൈവാനുഗ്രഹം',
    'premiumPerMonthCaption': 'മാസത്തിൽ',
    'premiumTrialLeadPrefix': 'നിങ്ങളുടെ 1 ദിവസത്തെ സൗജന്യ ട്രയൽ ആരംഭിക്കൂ ',
    'premiumRefundedBadge': 'ഉടനടി തിരികെ ലഭിക്കും',
    'premiumFeatureWallpapers': 'പരിധിയില്ലാത്ത HD വാൾപേപ്പറുകൾ',
    'premiumFeatureRingtones': 'ഭക്തിഗാന റിംഗ്‌ടോണുകൾ',
    'premiumFeatureDaily': 'എന്നും പുതിയ ഉള്ളടക്കം',
    'premiumCtaTrial': 'സൗജന്യ ട്രയൽ തുടങ്ങൂ',
    'premiumCtaSubscribe': 'ഇപ്പോൾ സബ്‌സ്ക്രൈബ് ചെയ്യൂ',
    'premiumReassuranceTrial':
        '₹2 പരിശോധന, ഉടനടി തിരികെ · എപ്പോൾ വേണമെങ്കിലും റദ്ദാക്കാം',
    'premiumReassurancePaid': 'UPI ഓട്ടോപേ സുരക്ഷ · ഒറ്റ ടാപ്പിൽ റദ്ദാക്കാം',
    'premiumSelectedUpiApp': 'UPI ആപ്പ്',
    'upiPickerTitle': 'ഇതുവഴി അടയ്ക്കൂ',
    'upiPickerLastUsed': 'അവസാനം ഉപയോഗിച്ചത്',
    'premiumTrialFinePrint':
        'പിന്നീട് മാസം {price} ഓട്ടോപേ വഴി. എപ്പോൾ വേണമെങ്കിലും റദ്ദാക്കാം.',
    'premiumSocialProof': '{city}-യിൽ {name} ലൈവ് വാൾപേപ്പർ ഇട്ടു 🙏',
  },
  'hi': {
    'appName': 'अरुल',
    'appTagline': 'दक्षिण भारतीय वॉलपेपर',
    'feedLiveBadge': 'लाइव',
    'feedEmptyTitle': 'यहाँ अभी कुछ नहीं है',
    'feedEmptyBody': 'कोई दूसरी श्रेणी आज़माएँ।',
    'feedErrorTitle': 'वॉलपेपर लोड नहीं हो सके',
    'feedErrorBody': 'अपना कनेक्शन जाँचें और फिर कोशिश करें।',
    'errorGeneric': 'कुछ गड़बड़ हो गई।',
    'signInCaption': 'दिव्यता आपके घर में',
    'signInGoogle': 'Google से जारी रखें',
    'signInSubtitleIdle': 'शुरू करने के लिए खाता चुनें',
    'signInSubtitleExchanging': 'साइन इन हो रहा है…',
    'signInNudgeBackedOutQuick': 'नहीं हुआ? फिर से टैप करें',
    'signInNudgeBackedOutSlow': 'एक बार टैप करें, Google का इंतज़ार',
    'signInNudgeNeverOpened': 'Google नहीं खुला, फिर से टैप करें',
    'signInNudgeAddAccount': 'इस फ़ोन का मौजूदा खाता चुनें',
    'signInNudgeReauth': 'फिर से टैप करें और खाता चुनें',
    'signInNudgeActivityClosed': 'साइन इन जल्दी बंद हुआ, फिर टैप करें',
    'signInNudgeNoProvider': 'पहले Google Play सेवाएं अपडेट करें',
    'signInFixBackedOutSlow': 'इस फ़ोन पर Google थोड़ा समय ले रहा है।',
    'signInFixAddAccount':
        'नया खाता ज़रूरी नहीं। इस फ़ोन पर मौजूद कोई भी Google खाता चलेगा।',
    'signInFixReauth':
        'Google उस खाते की पुष्टि नहीं कर सका। फ़ोन सेटिंग में Google में दोबारा साइन इन करें।',
    'signInFixActivityClosed': 'Google Play सेवाओं ने साइन इन विंडो बंद कर दी।',
    'signInLinkAccountSettings': 'खाता सेटिंग खोलें',
    'signInLinkPlayServices': 'Google Play सेवाएं देखें',
    'signInLinkPlayStore': 'Play Store में अपडेट करें',
    'signInTermsLink': 'शर्तें',
    'signInPrivacyLink': 'गोपनीयता',
    'premiumTitle': 'प्रीमियम',
    'premiumHeadline': 'हर वॉलपेपर अनलॉक करें',
    'premiumSub':
        'ब्राउज़ करना हमेशा मुफ़्त है। प्रीमियम उन्हें अपना बनाने के लिए है।',
    'premiumBenefitApply': 'कोई भी वॉलपेपर लगाएँ — स्थिर या लाइव',
    'premiumBenefitLive': 'पूरी गुणवत्ता में लाइव वीडियो वॉलपेपर',
    'premiumBenefitShare': 'दोस्तों और परिवार के साथ साझा करें',
    'premiumBenefitNew': 'हर हफ़्ते नए वॉलपेपर',
    'premiumPrice': '₹199 / माह',
    'premiumCta': 'मुफ़्त ट्रायल शुरू करें',
    'premiumTrialNote':
        'प्रति खाता एक मुफ़्त ट्रायल। कभी भी रद्द करें — अवधि समाप्त होने तक पहुँच बनी रहती है।',
    'premiumComingSoon': 'भुगतान बाद में आएँगे।',
    'settingsTitle': 'सेटिंग्स',
    'settingsAppearance': 'रूप',
    'themeSystem': 'सिस्टम',
    'themeLight': 'लाइट',
    'themeDark': 'डार्क',
    'settingsContent': 'सामग्री',
    'settingsAbout': 'बारे में',
    'settingsSupport': 'मदद चाहिए',
    'settingsPrivacy': 'गोपनीयता नीति',
    'settingsVersion': 'संस्करण',
    'uploadBody': 'अपना वॉलपेपर साझा करें। हम हर सबमिशन की समीक्षा करते हैं।',
    'uploadPickCategory': 'श्रेणी चुनें',
    'uploadPickFile': 'फ़ाइल चुनें',
    'uploadSpecNote':
        'फ़ोटो: 1080×1920. वीडियो: 1024×1824, बिना ऑडियो, 50 MB से कम।',
    'uploadComingSoon': 'अपलोड बाद में आएँगे।',
    'apply': 'लगाएँ',
    'share': 'साझा करें',
    'applyTargetTitle': 'वॉलपेपर लगाएँ',
    'applyTargetHome': 'होम स्क्रीन',
    'applyTargetLock': 'लॉक स्क्रीन',
    'applyTargetBoth': 'होम और लॉक स्क्रीन',
    'applied': 'वॉलपेपर लगाया गया',
    'appliedLiveFallback':
        'यह फ़ोन लाइव वॉलपेपर नहीं चला सकता, इसलिए हमने एक स्थिर तस्वीर लगा दी।',
    'offlineBody': 'आप ऑफ़लाइन हैं। अपना कनेक्शन जाँचें और फिर कोशिश करें।',
    'offlineTitle': 'इंटरनेट नहीं है',
    'offlineFeedBody': 'वॉलपेपर देखने के लिए इंटरनेट चालू करें।',
    'wallpaperShareCaption':
        'इस जैसे और भक्ति वॉलपेपर — स्थिर और लाइव — Arul पर:\n{link}',
    'referShareMessage':
        'दक्षिण भारतीय भक्ति वॉलपेपर के लिए मुझे Arul बहुत पसंद है — अम्मन, मुरुगन, पेरुमाल, शिवन, और चलने वाले लाइव वॉलपेपर भी। सोचा आपको भी पसंद आएगा।\n\n{link}',
    'tabWallpapers': 'वॉलपेपर',
    'ringtonePreviewSemantic': 'प्रीव्यू',
    'ringtonePreviewUnavailable': 'प्रीव्यू अभी उपलब्ध नहीं है',
    'ringtonesEmptyTitle': 'रिंगटोन जल्द आ रहे हैं',
    'ringtonesEmptyBody':
        'भक्ति रिंगटोन रास्ते में हैं। कुछ समय बाद फिर देखें।',
    'ringtonesErrorTitle': 'रिंगटोन लोड नहीं हो सके',
    'ringtoneSetSuccess':
        'रिंगटोन सेट हो गया। दिखाई न दे तो फ़ोन रीस्टार्ट करें।',
    'ringtoneSetFailed': 'रिंगटोन सेट नहीं हो सका। फिर से कोशिश करें।',
    'ringtoneSetPreparing': 'तैयार हो रहा है…',
    'ringtoneSetDownloading': 'डाउनलोड हो रहा है…',
    'ringtoneSetApplying': 'रिंगटोन सेट हो रहा है…',
    'save': 'सेव करें',
    'cancel': 'रद्द करें',
    'errorGenericRetry': 'कुछ गड़बड़ हो गई। फिर कोशिश करें।',
    'settingsPremiumSubLocked': 'लगाना और साझा करना अनलॉक करें',
    'settingsPremiumSubTrial': 'आप मुफ़्त ट्रायल पर हैं',
    'settingsPremiumSubCancelled': 'ऑटो-रिन्यू बंद · पहुँच जारी है',
    'settingsPremiumSubActive': 'आप सदस्य हैं',
    'settingsReferSub': '30 दिन मुफ़्त प्रीमियम कमाएँ',
    'settingsTellFriend': 'दोस्त को बताएँ',
    'settingsTellFriendSub': 'जिसे Arul पसंद आएगा, उसे भेजें',
    'settingsRemindersSubOn': 'साप्ताहिक और त्योहार रिमाइंडर चालू',
    'settingsRemindersSubOff': 'त्योहार और साप्ताहिक रिमाइंडर',
    'settingsLanguage': 'भाषा',
    'settingsTheme': 'थीम',
    'settingsNeedHelp': 'मदद चाहिए?',
    'settingsNeedHelpSub': 'सपोर्ट से संपर्क करें',
    'settingsUpload': 'अपनी सामग्री अपलोड करें',
    'settingsUploadSub': 'वॉलपेपर या रिंगटोन साझा करें',
    'settingsLogout': 'लॉग आउट',
    'settingsLogoutConfirmTitle': 'लॉग आउट करें?',
    'settingsLogoutConfirmBody':
        'आप कभी भी Google से दोबारा साइन इन कर सकते हैं।',
    'settingsDeleteAccount': 'खाता मिटाएँ',
    'settingsDeleteConfirmTitle': 'खाता मिटाएँ?',
    'settingsDeleteConfirmBody':
        'इससे आपका खाता, पसंदीदा और रिवॉर्ड हमेशा के लिए हट जाएँगे।',
    'settingsTerms': 'नियम और शर्तें',
    'settingsRefund': 'रिफ़ंड नीति',
    'settingsSupportEmailPrompt': 'कृपया अपनी समस्या या सुझाव लिखें:',
    'settingsSupportEmailDetails':
        'नीचे दी गई जानकारी आपका अनुरोध जल्दी हल करने में मदद करती है:',
    'settingsNoEmailApp': 'कोई ईमेल ऐप नहीं मिला। हमें {email} पर लिखें',
    'settingsEditNameTitle': 'आपका नाम',
    'themeSystemDefault': 'सिस्टम डिफ़ॉल्ट',
    'themeSystemSub': 'डिवाइस सेटिंग के अनुसार',
    'themeLightSub': 'आइवरी और रेशम',
    'themeDarkSub': 'दीप-प्रकाश मैरून',
    'remindersToggleTitle': 'भक्ति रिमाइंडर',
    'remindersToggleSub': 'साप्ताहिक दिन, और हर बड़ा त्योहार',
    'remindersScheduleNote':
        'हर हफ़्ते शुक्रवार को एक रिमाइंडर, और हर बड़े त्योहार से कुछ दिन पहले एक — पोंगल, दीपावली, नवरात्रि, शिवरात्रि वगैरह। महीने में करीब दो।',
    'remindersPermissionToast':
        'Arul के लिए नोटिफ़िकेशन बंद हैं। रिमाइंडर पाने के लिए फ़ोन सेटिंग्स में चालू करें।',
    'remindersTimeLabel': 'रिमाइंडर का समय',
    'remindersComingUp': 'आने वाले',
    'remindersMonthJan': 'जन',
    'remindersMonthFeb': 'फ़र',
    'remindersMonthMar': 'मार्च',
    'remindersMonthApr': 'अप्रैल',
    'remindersMonthMay': 'मई',
    'remindersMonthJun': 'जून',
    'remindersMonthJul': 'जुल',
    'remindersMonthAug': 'अग',
    'remindersMonthSep': 'सित',
    'remindersMonthOct': 'अक्टू',
    'remindersMonthNov': 'नव',
    'remindersMonthDec': 'दिस',
    'premiumBrandTitle': 'Arul Premium',
    'premiumScreenSubline': 'पूरा संग्रह, आपकी स्क्रीन पर जीवंत',
    'premiumPerkEvery': 'हर वॉलपेपर — स्थिर और लाइव',
    'premiumPerkApplyShare': 'बिना किसी सीमा के लगाएँ और साझा करें',
    'premiumPerkNew': 'हर हफ़्ते नए वॉलपेपर',
    'premiumPerMonth': '/ माह',
    'premiumPlanNote': 'UPI ऑटोपे · कभी भी रद्द करें',
    'premiumTrialPill': '1 दिन मुफ़्त',
    'premiumCtaPaid': 'प्रीमियम लें',
    'premiumFootnoteTrial':
        '1 दिन मुफ़्त, फिर {price}/माह। UPI ऑटोपे आपका खाता ₹2 से जाँचता है, जो तुरंत वापस हो जाता है। ब्राउज़ करना हमेशा मुफ़्त रहेगा।',
    'premiumFootnotePaid':
        'आज {price} लिया जाएगा, फिर UPI ऑटोपे से हर महीने रिन्यू होगा। कभी भी रद्द करें। ब्राउज़ करना हमेशा मुफ़्त रहेगा।',
    'premiumComingSoonToast': 'प्रीमियम जल्द आ रहा है।',
    'premiumWelcomeToast': 'Arul Premium में आपका स्वागत है!',
    'premiumCelebrateTitle': 'आप जुड़ गए',
    'premiumCelebrateBody':
        'Arul Premium चालू है। किसी को ये वॉलपेपर पसंद आएँगे? उन्हें एक भेजें।',
    'premiumSheetPitch':
        'हर वॉलपेपर — लाइव और स्थिर। छहों श्रेणियों में बेरोक लगाएँ और साझा करें।',
    'premiumKeepBrowsing': 'मुफ़्त में देखते रहें',
    'referHeroTitle': 'दोस्त को तोहफ़ा, आपको एक महीना',
    'referHeroBody':
        'आपकी लिंक से सब्सक्राइब करने वाले हर दोस्त पर 30 दिन मुफ़्त प्रीमियम',
    'referRewardsLabel': 'कमाए रिवॉर्ड',
    'referRewardDays': '{days} दिन',
    'referStep1': 'अपनी लिंक दोस्तों और परिवार के साथ साझा करें',
    'referStep2': 'वे Arul इंस्टॉल करके प्रीमियम लेते हैं',
    'referStep3': '30 दिन मुफ़्त प्रीमियम आपके खाते में आ जाता है',
    'referEmpty': 'अभी कोई रेफ़रल नहीं — एक शेयर में पहला दोस्त',
    'referShareCta': 'Arul साझा करें',
    'referNotNow': 'अभी नहीं',
    'uploadScreenTitle': 'वॉलपेपर अपलोड',
    'uploadPickZoneSub': 'पोर्ट्रेट, 1080×2400 या बड़ा',
    'uploadTitleLabel': 'शीर्षक',
    'uploadTitleOptional': '(वैकल्पिक)',
    'uploadCategoryLabel': 'श्रेणी',
    'uploadRightsCheckbox':
        'इस सामग्री के अधिकार मेरे हैं, या मुझे इसे साझा करने की अनुमति है',
    'uploadFootnote': 'मंज़ूर वॉलपेपर आपके नाम के साथ फ़ीड में दिखेंगे',
    'uploadRejectStatic': 'JPEG, PNG या WebP तस्वीर चुनें।',
    'uploadRejectLive': 'MP4 वीडियो चुनें।',
    'uploadRejectAudio': 'MP3, AAC या M4A ऑडियो फ़ाइल चुनें।',
    'uploadKindLabel': 'आप क्या साझा कर रहे हैं?',
    'uploadKindWallpaper': 'वॉलपेपर',
    'uploadKindRingtone': 'रिंगटोन',
    'uploadPickZoneTitleAudio': 'ऑडियो फ़ाइल चुनें',
    'uploadPickZoneSubAudio': 'MP3, AAC या M4A',
    'uploadTitleHintRingtone': 'जैसे, कंद षष्ठी कवचम',
    'uploadFootnoteRingtone':
        'मंज़ूर रिंगटोन आपके नाम के साथ रिंगटोन टैब में दिखेंगे',
    'uploadShareMomentBodyRingtone':
        'हम आपके रिंगटोन की जल्द समीक्षा करेंगे। तब तक — किसी को Arul पसंद आएगा?',
    'uploadTooLarge': 'फ़ाइल बहुत बड़ी है (अधिकतम {max})।',
    'uploadSuccessToast': 'समीक्षा के लिए भेज दिया — धन्यवाद!',
    'uploadShareMomentTitle': 'धन्यवाद',
    'uploadShareMomentBody':
        'हम आपके वॉलपेपर की जल्द समीक्षा करेंगे। तब तक — किसी को Arul पसंद आएगा?',
    'uploadComingSoonToast': 'अपलोड जल्द आ रहा है।',
    'premiumNavTitle': 'सदस्यता',
    'premiumEyebrow': 'प्रीमियम',
    'premiumTagline': 'हर दिन दिव्य कृपा',
    'premiumPerMonthCaption': 'प्रति माह',
    'premiumTrialLeadPrefix': 'अपना 1 दिन का मुफ़्त ट्रायल शुरू करें ',
    'premiumRefundedBadge': 'तुरंत वापस',
    'premiumFeatureWallpapers': 'असीमित HD वॉलपेपर',
    'premiumFeatureRingtones': 'भक्ति रिंगटोन',
    'premiumFeatureDaily': 'हर दिन नया कंटेंट',
    'premiumCtaTrial': 'मुफ़्त ट्रायल शुरू करें',
    'premiumCtaSubscribe': 'अभी सदस्यता लें',
    'premiumReassuranceTrial': '₹2 सत्यापन, तुरंत वापस · कभी भी रद्द करें',
    'premiumReassurancePaid': 'UPI ऑटोपे सुरक्षित · एक टैप में रद्द करें',
    'premiumSelectedUpiApp': 'UPI ऐप',
    'upiPickerTitle': 'इससे भुगतान करें',
    'upiPickerLastUsed': 'पिछली बार इस्तेमाल',
    'premiumTrialFinePrint': 'फिर {price}/माह ऑटोपे से। कभी भी रद्द करें।',
    'premiumSocialProof': '{city} में {name} ने लाइव वॉलपेपर लगाया 🙏',
  },
};

/// Keys whose message takes ICU placeholders, with their names.
/// Their rendered text never equals the template, so the matrix
/// matches them by turning the template into a regex.
const Map<String, List<String>> kArbPlaceholders = {
  'premiumFootnotePaid': ['price'],
  'premiumFootnoteTrial': ['price'],
  'premiumSocialProof': ['name', 'city'],
  'premiumTrialFinePrint': ['price'],
  'referRewardDays': ['days'],
  'referShareMessage': ['link'],
  'settingsNoEmailApp': ['email'],
  'uploadTooLarge': ['max'],
  'wallpaperShareCaption': ['link'],
};
